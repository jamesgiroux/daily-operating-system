...
export default function AccountDetailPage() {
  ...
  const renderWorkView = () => {
    ...
    //     (Linear/REDACTED/Slack appear in the source string). Else private.
    ...
    const commitmentCards = openCommitments.map((c, idx) => {
      ...
      const isShared = /linear|REDACTED|slack|jira|asana|dos-|opp/i.test(c.source ?? "");
    ...
    // (DOS-75 will wire real Linear/REDACTED/Slack feeds in v1.2.2).
    const sharedItems = openCommitments.filter((c) =>
      /linear|REDACTED|slack|jira|asana|dos-|opp/i.test(c.source ?? ""),
    );
    ...
    const oldestPrivate = openCommitments
      .filter((c) => !/linear|REDACTED|slack|jira|asana|dos-/i.test(c.source ?? ""))
    ...
    const staleShared = openCommitments
      .filter((c) => /linear|REDACTED|slack|dos-/i.test(c.source ?? ""))
      .find((c) => {
