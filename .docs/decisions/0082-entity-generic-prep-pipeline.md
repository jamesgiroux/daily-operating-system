...
A user linked a meeting from "REDACTED" (account) to "Agentforce" (project). Making that change stick required fixes in four separate files across five code paths:
...
```json
{
  ...
  "entities": [
    ...
    { "id": "jefferies", "type": "account", "name": "Jefferson Advisors" }
  ],
  "entityContext": { ... },
  "attendees": [ ... ]
}
