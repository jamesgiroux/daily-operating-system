...
- [ADR-0100](0100-glean-first-intelligence-architecture.md) introduces `local_enrichment` and describes Glean as a multi-source feeder from REDACTED, Zendesk, Gong, Slack, P2, and org directories.
- [ADR-0105](0105-provenance-as-first-class-output.md) §4 references `DataSource` in provenance attribution, including REDACTED and others not in ADR-0098's enum.
...
**Glean is modeled as an aggregator, not as a single source.** When an ability attributes `DataSource::Glean { downstream: REDACTED }`, it captures both the retrieval path (Glean) and the originating source (REDACTED). This preserves ADR-0100's Glean-first posture without erasing downstream lineage needed for trust and lifecycle decisions.
...
| `Glean { downstream: REDACTED }` | Scoring | CRM is the system of record |
...
The `scoring_class` field on `SourceAttribution` ([ADR-0105](0105-provenance-as-first-class-output.md) §4) is derived from this table, not author-set. Consumers that aggregate sources for scoring filter by class to avoid conflating Slack chatter with REDACTED ARR.
...
4. **Scoring class eliminates source conflation.** Slack chatter and REDACTED ARR are distinguishable at attribution time.
...
1. **Glean downstream misattribution.** An ability attributes content as `Glean { downstream: REDACTED }` when it was actually from Zendesk. Mitigation: Glean API provides downstream metadata; abilities use it directly, not inferred.
