...
Maeve Lander (Glean adoption lead at VIP) positioned DailyOS as a "presentation and interaction layer" on top of Glean's knowledge graph. For VIP/Automattic, Glean already indexes REDACTED, Gong, Zendesk, Confluence, Gmail, and LinkedIn — the same sources DailyOS currently reaches via separate connectors (Clay, Gravatar, Gmail API).
