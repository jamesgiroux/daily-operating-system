...
pub struct ExternalClients<'a> {
    ...
    pub REDACTED: SalesforceWrapper<'a>,
    // ...
}
```
...
**External calls are mode-aware.** Services that call external APIs (Glean, Slack, Gmail, REDACTED) check mode at the call boundary and route to either the live client (`Live`) or a replay fixture supplied by the `EvalFixture` (`Simulate`/`Evaluate`). The replay fixture API is specified in [ADR-0107](0107-evaluation-harness-for-abilities.md).
...
4. **External-call routing adds a mode branch to every integration.** Glean, Slack, Gmail, REDACTED wrappers each gain a mode check and a fixture replay path.
...
4. **External-call replay interface is load-bearing but specified elsewhere.** Services that call Glean, Slack, Gmail, REDACTED must route through a replay interface in `Simulate`/`Evaluate`. The interface specification lives in [ADR-0107](0107-evaluation-harness-for-abilities.md). Until 0107 lands, the external-call routing in §3.3 is contract-only; tests cannot actually run in `Evaluate` mode against external-dependent abilities.
