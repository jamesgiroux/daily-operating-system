...
**Exception — outbox pattern for external mutations.** Abilities that must span a local DB write and an external API call (e.g., `sync_to_REDACTED`, `publish_to_p2` when invoked by system policy) cannot be transactional across both. Instead, they use an outbox pattern:
