...
**Context:** I559 validation spike confirmed Glean MCP `chat` tool produces structured JSON matching I508 schema with real cross-source data (Zendesk, REDACTED, Gong, Slack, org chart) in 10-30 seconds.
...
2. It accesses REDACTED (CRM), Zendesk (support), Gong (call recordings), Slack, P2, and org directories simultaneously
...
| Account discovery | Manual entry during onboarding | Automatic from Glean (REDACTED + Gong + Zendesk cross-reference) |
| People enrichment | Clay + Gravatar | Glean org chart + REDACTED contacts + Gong participants |
...
|---|---|---|---|
| REDACTED (CRM) | 0.9 | System of record. Renewal dates, ARR, deal stage, ownership. | Direct input to `financialProximity`. Renewal probability feeds `signalMomentum`. |
...
**Why:** A Slack thread about an account could mean anything — an FYI, a celebration, a crisis, or idle speculation. Unlike a Zendesk ticket (which represents a real customer interaction) or a REDACTED field (which represents a committed data point), Slack is ambient internal awareness.
...
**The principle:** Slack tells you what your team knows. Zendesk tells you what the customer experienced. Gong tells you what was said. REDACTED tells you what's committed. Each has a different reliability weight.
...
compute_stakeholder_coverage()
  + REDACTED contact list for the account (verifies coverage against actual org)
...
compute_financial_proximity()
  + REDACTED renewal stage, probability %, deal value
