...
Ten Gong transcripts of the same meeting are not ten sources. Three independent sources (REDACTED + Zendesk + Gong) asserting the same thing is stronger evidence than a hundred same-source repetitions. The corroboration math reflects this: each `(claim_id, data_source)` edge accumulates strength with diminishing returns, and claim-level confidence aggregates via noisy-OR over independent edges.
