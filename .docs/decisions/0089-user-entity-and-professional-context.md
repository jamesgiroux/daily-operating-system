...
| `current_priorities` | What you're focused on this quarter or period | "Expand the Cox account to two new business units. Close Jefferies before Q2." |
...
When `current_priorities` contains "expand Cox account", signals from Cox-domain entities get a relevance multiplier. When `value_proposition` mentions security compliance, security-related signals from accounts rank higher in the briefing's attention section.
