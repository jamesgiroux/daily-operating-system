...
The current entity model treats all accounts as structurally equivalent: a parent account is a container that holds child accounts. It has no intelligence of its own beyond what's manually entered. A user working across 10 REDACTED BUs under one parent company has no surface that shows them the portfolio view — what's happening across all 10 at once, whether a pattern appearing in three BUs is worth escalating, which BUs need attention today vs. which can wait.
...
Parent-level meetings compound the problem. A meeting tagged to "Crestview Media Enterprises" might contain signals about Crestview Media B2B, Crestview Media Retail, and the parent entity simultaneously. Currently the signal from that meeting lands on one entity (whichever the meeting is tagged to) and stays there.
...
When a parent-level meeting contains signals for multiple BUs ("for Crestview Media B2B we're doing X, for Crestview Media Retail we're doing Y"), the correct behavior is to extract and route signals to the appropriate child entities. This requires content-level entity resolution in the transcript processor — a meaningful pipeline addition.
