...
| Eucalyptus | `--color-garden-eucalyptus` | `#6ba8a4` | User entity (/me), professional context, self-knowledge |
...
| User (/me) | Eucalyptus | `--color-entity-user` |
...
**Resolved:** Opacity token variants added to `design-tokens.css` by I447 (e.g., `--color-spice-turmeric-8`, `--color-spice-turmeric-10`, `--color-spice-turmeric-12`, `--color-garden-larkspur-12`). Use these instead of scattered rgba values.
...
2. Add route in `src/router.tsx` + add to `MAGAZINE_ROUTE_IDS` if editorial
3. Register magazine shell via `useRegisterMagazineShell()`
4. Handle loading/error/empty states (see [STATE-PATTERNS.md](./STATE-PATTERNS.md) for patterns, decision tree, and per-page hook reference)
5. End with `<FinisMarker />`
6. Add to [PAGE-ARCHITECTURE.md](./PAGE-ARCHITECTURE.md)
7. Add to Compliance Summary

### Adding a New Component

1. Check [COMPONENT-INVENTORY.md](./COMPONENT-INVENTORY.md) — does something similar already exist?
2. Use design tokens for all colors, spacing, typography
3. Use CSS modules for complex styles, Tailwind for simple ones
4. No inline `style={{}}` props
