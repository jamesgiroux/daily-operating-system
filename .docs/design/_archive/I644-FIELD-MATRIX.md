...
- **fact** -- system-backed value from a source of record (REDACTED, Zendesk). Safe to promote into canonical account state.
...
| REDACTED (via Glean CRM) | 0.9 | glean_crm |
...
|-------|-------------|-------------|----------------|-----------|-----|
| ARR (system value) | REDACTED | fact | `accounts.arr` + `account_source_refs` | 24h | 5, 8 |
| ARR (AI estimate) | Glean synthesis | inference | `account_source_refs` only | 72h | 9 |
| ARR range low | REDACTED | fact | `accounts.arr_range_low` (not yet added) | 24h | 5 |
| ARR range high | REDACTED | fact | `accounts.arr_range_high` (not yet added) | 24h | 5 |
| Contract start | REDACTED | fact | `contract_context.contract_start` (entity_assessment) | 30d | 5 |
| Contract end / renewal date | REDACTED | fact | `accounts.contract_end` + `account_source_refs` | 24h | 5 |
| Contract type | REDACTED | fact | `contract_context.contract_type` (entity_assessment) | 30d | 7 |
| Auto-renew | REDACTED | fact | `contract_context.auto_renew` (entity_assessment) | 30d | 7 |
| Multi-year remaining | REDACTED | fact | `contract_context.multi_year_remaining` (entity_assessment) | 30d | 7 |
| Previous renewal outcome | REDACTED / Glean | signal | `contract_context.previous_renewal_outcome` (entity_assessment) | 90d | 7 |
| Customer fiscal year start | REDACTED | fact | `contract_context.customer_fiscal_year_start` (entity_assessment) | 90d | 7 |
| Procurement notes | Glean synthesis | inference | `contract_context.procurement_notes` (entity_assessment) | 30d | 9 |
| Renewal likelihood | REDACTED | fact | `accounts.renewal_likelihood` (not yet added) | 24h | 5, 6 |
| Renewal opportunity stage | REDACTED | fact | `accounts.commercial_stage` (migration 076) | 24h | 6, 11 |
...
| Expansion signals | Gong / REDACTED / Glean | signal | `expansion_signals[]` (entity_assessment dimensions_json) | 30d | 7 |
| Expansion ARR impact | REDACTED | signal | `expansion_signals[].arr_impact` (entity_assessment) | 30d | 7 |
| Blockers | Gong / meetings | signal | `blockers[]` (entity_assessment dimensions_json) | 14d | 7 |
| Product classification | REDACTED | fact | `product_classification.products[]` (entity_assessment) + `account_products` table | 14d | 5 |
| Product type | REDACTED | fact | `account_products.product_type` (I651) | 14d | 5 |
| Product tier | REDACTED | fact | `account_products.tier` (I651) | 14d | 5 |
| Product ARR | REDACTED | fact | `account_products.arr` (I651) | 14d | 5 |
| Product billing terms | REDACTED | fact | `account_products.billing_terms` (I651) | 14d | 5 |
...
| Field | Source System | Source Kind | DB Destination | Freshness | AC |
|-------|-------------|-------------|----------------|-----------|-----|
| Stakeholder name | REDACTED / Gong / Glean | fact | `account_stakeholders.person_id` + `people.name` | 30d | 7 |
| Stakeholder role (job title) | REDACTED / Glean | fact | `people.role` | 30d | 7 |
...
| Internal team members | REDACTED / Glean | fact | `internal_team[]` (entity_assessment dimensions_json) | 30d | 7 |
...
| Field | Source System | Source Kind | DB Destination | Freshness | AC |
|-------|-------------|-------------|----------------|-----------|-----|
| Support tier / package | Zendesk / REDACTED | fact | `account_technical_footprints.support_tier` (I649) | 7d | 5 |
...
| Active subscription count | REDACTED | fact | not yet extracted -- derive from `account_products` count | 14d | 5 |
| Primary product | REDACTED | fact | not yet extracted -- derive from highest-ARR `account_products` row | 14d | 5 |
| Customer status | REDACTED | fact | not yet extracted -- maps to `accounts.lifecycle` | 7d | 5 |
...
| Customer stage | REDACTED | fact | `org_health.customer_stage` (entity_assessment) | 7d | 7 |
...
| Recent wins | Gong / REDACTED / Glean | signal | `entity_assessments.wins_json` (DB) | 30d | 9 |
...
| Company industry | Glean / REDACTED | fact | `company_context.industry` (entity_assessment) | 90d | 7 |
| Company size | Glean / REDACTED | fact | `company_context.size` (entity_assessment) | 90d | 7 |
...
|-------|----------------|---------------------|--------|
| `arr_range_low` | REDACTED | `accounts.arr_range_low` | Not yet added to schema |
| `arr_range_high` | REDACTED | `accounts.arr_range_high` | Not yet added to schema |
| `renewal_likelihood` | REDACTED | `accounts.renewal_likelihood` | Not yet added to schema |
| `renewal_model` | REDACTED | new column | Not yet added to schema |
| `renewal_pricing_method` | REDACTED | new column | Not yet added to schema |
...
| `customer_status` | REDACTED | maps to `accounts.lifecycle` | Not yet extracted directly |
