...
**Date:** 2026-03-02
**Status:** Research — blocked on Glean/REDACTED authentication validation
...
This is produced manually each week. Someone aggregates REDACTED data, calls RMs for status updates, computes WoW deltas against last week's numbers, and formats a Slack message.

## Three Data Layers

| Layer | What | Source | DailyOS Can Provide? |
|-------|------|--------|---------------------|
| **Structured financials** | ARR, budget targets, pipeline stages, product line | REDACTED (CRM) | Only via Glean — blocked today |
...
## Glean/REDACTED Blocker

**Status:** Glean can query REDACTED via SOQL, but REDACTED authentication is not configured in the current workspace.

**What Glean reported:**
- Glean has an internal REDACTED Search action that accepts SOQL-style queries
- The action returned `401 "SoqlAction is not authenticated"`
- This is a one-time workspace-level authentication setup — once connected, Glean can pull ARR, pipeline stages, opportunity amounts, and account metadata from REDACTED
...
- If REDACTED is authenticated in Glean, DailyOS could query Glean for CRM data to populate the financial scaffolding
- The structured financial data (ARR, budget, pipeline stages) would come from Glean's REDACTED connector
- DailyOS adds the intelligence narrative on top
- Without REDACTED in Glean, DailyOS can only work with manually entered vitals (ARR, renewal date) — insufficient for the full report

**Validation gate:** Before scoping this as an issue, confirm:
1. Can REDACTED be authenticated in Glean for this workspace?
2. What fields does Glean surface from REDACTED? (ARR, product line, pipeline stage, opportunity amount, close date, attrition flag)
...
2. **Product line segmentation** — accounts need a product line or business unit attribute. This could be a tag, a custom field, or inferred from Glean/REDACTED data.
...
4. **Budget targets** — quarterly budget by product line. Could be a user-entered configuration or pulled from REDACTED.
...
**Park as a future report type.** The financial scaffolding is gated on Glean/REDACTED authentication — without it, the report can't be produced at the fidelity shown in the example. Once the Glean validation gate is cleared:

1. If REDACTED data is available via Glean: scope as a v1.2.0 report type that combines CRM aggregates with DailyOS intelligence narrative
2. If REDACTED data is not available: scope as a lighter "Weekly Intelligence Update" that tracks health movements, saves, surprises using DailyOS-native data only (less financial precision, more relationship intelligence)
