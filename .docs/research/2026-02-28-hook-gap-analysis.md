...
| Org-wide context (Zendesk, REDACTED, Confluence, Gong) | Glean integration — pulls from all Glean-indexed systems | v0.15.2 (dev) | **Covered via Glean.** Glean indexes these sources; DailyOS synthesizes them into intelligence. |
...
Glean is a retrieval engine. It finds what exists across indexed sources. "What tickets does Acme have open?" "When was the last Gong call?" "What does the account plan say?" Those are lookups against documents that already exist somewhere in REDACTED, Zendesk, Confluence, or Gong. Glean is excellent at this because it has the connectors, the permissions model, and the search infrastructure.

But "Is Acme at risk?" is not a retrieval question. There is no document in REDACTED or Confluence that contains the sentence "Acme is at risk." Risk is an *interpretation* — it emerges from synthesizing multiple signals across multiple systems:
...
In Glean mode, there's an untapped opportunity: **Glean-driven account discovery.** Glean indexes REDACTED, so it knows every account in the org's CRM. A "Discover from Glean" flow could:
...
- **@Glean in Slack** — a conversational AI assistant that answers questions from the full Glean-indexed corpus (REDACTED, Zendesk, Confluence, Gong, Jira, etc.)
...
This means a VP could already say "@Glean what do we know about Acme?" in Slack and get an answer drawn from REDACTED records, Zendesk tickets, Gong calls, and Confluence docs. The question is: **what does DailyOS add that Glean alone doesn't provide?**

### What Glean's Slack Bot Knows vs. What DailyOS Knows

| Data Layer | Glean Knows | DailyOS Knows |
|---|---|---|
| CRM records (REDACTED) | Yes | No (Glean-sourced) |
...
1. **Evaluate Glean's Slack bot for VP queries.** Glean already has @Glean in Slack with full access to REDACTED, Zendesk, Gong, Confluence. Test whether it answers the VP's most common questions ("what's going on with Acme?", "any open tickets?", "when was the last Gong call?"). If it does, the "conversational interface" gap is already partially closed — for free.
...
- [Glean + REDACTED AI in Slack](https://www.glean.com/blog/glean-slack-ai-app-container)
