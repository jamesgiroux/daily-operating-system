...
The Glean MCP `chat` tool can fully replace PTY calls to Claude Code for entity intelligence enrichment. It returns structured JSON matching our I508 schema, with data from Zendesk, REDACTED, Gong, Slack, and internal docs that local-only enrichment cannot access. Response time is 10-30 seconds vs 60-180 seconds for PTY calls.
...
**Key data sources for CS intelligence:** REDACTED (salescloud/servicecloud), Zendesk, Gong, P2 (internal comms), Zoom.
...
- **Status:** PASS — valid JSON
- **Data quality:** Empty stakeholders for Globex Holdings external contacts (Glean may not have external contact data), but correctly identified no champion
...
- **Accounts found:** Nexus Analytics, REDACTED, Meridian Asset, Crestview Media, REDACTED, AirBnB, REDACTED, Globex Holdings, Keystone Finance, Newsweek, CEI, Akismet
- **Notable:** Knows role per account (TAM, co-owner, technical side)

### Test 9: Full Onboarding Bootstrap
- **Status:** PASS — valid JSON
- **Data quality:** 2 REDACTED-owned accounts with health bands, renewal data ($105K at 30% probability), stakeholders, last activity
...
| People enrichment | Clay + Gravatar | Org chart + internal team + REDACTED contacts |
| Account discovery | Manual entry | Automatic from REDACTED/Gong/Zendesk |
...
- Health scoring dimensions augmented with Glean data (support tickets → `supportHealth` dimension, REDACTED data → `financialProximity`)
