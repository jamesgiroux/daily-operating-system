...
**Great Expectations** (data quality framework) implements what it calls "expectation suites" — declarative rules about what the data should look like. When a row violates an expectation, a remediation action fires. For DailyOS: "if intelligence mentions a company name not in the entity's account_domains, flag it" is an expectation that would catch the Jefferson Advisors/Adobe Fonts bug.
