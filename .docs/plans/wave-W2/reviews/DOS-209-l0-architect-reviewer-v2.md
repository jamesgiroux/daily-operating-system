...
Closure location in v2: §3 paragraph 2 ("`ExternalClients` is a concrete struct with named wrapper fields `glean`, `slack`, `gmail`, and `REDACTED`; wrapper internals hold live clients only in Live and replay/fixture wrappers in Simulate/Evaluate.") and §4 ("external replay wrappers must not contain live secrets in non-Live modes.").
