...
ADR-0104 §"Known Limitations" #5 is explicit: `IntelligenceProvider` lives on `AbilityContext`, **not** `ServiceContext`, while `ExternalClients` (Glean, Slack, Gmail, REDACTED) lives on `ServiceContext`. Mode-aware routing of LLM calls is therefore an `AbilityContext` concern; the service layer only carries the queue handle.
