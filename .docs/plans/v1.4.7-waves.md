...
File number unchanged. Internal cross-refs were updated on 2026-05-19 to match the 2026-05-17 Linear renumber: references to **v1.4.5** in this doc are **Workspace Memory** (was v1.4.6); references to **v1.4.6** are **Salience & Recommendations** (was v1.4.5). `v1.4.7 implementation gates on v1.4.5 merge` (workspace memory). Canonical map at `.docs/plans/wp-foundation-roadmap-reorientation.md` §"v1.4.x active sequence (post-renumber, 2026-05-17)".
...
**Class C — Scope naming convention frozen + reconciled with v1.4.5.**
8. **Canonical convention: `<dot-namespace>.<verb>.<noun>`** — e.g., `dailyos.read.account_status`, `dailyos.write.place_document`, `dailyos.submit.note`. v1.4.5 already shipped `write.workspace_place_document` for the AI placement ability; v1.4.7 W3-B's MCP tool reuses that exact scope verbatim (no rename). Scope-mapping table added to W0. Closes Challenge HIGH #5 + L0 question #3.
...
11. **Shared `Page<T>` envelope** added to W1-A `contracts.rs`. v1.4.5 `WorkspaceGraphResponse v1` is the documented pass-through exception (since v1.4.7 W3-A surfaces it unchanged for AI consumers).
...
Class A propagation hit AGAIN (third v1.4.7 cycle, sixth time across v1.4.5+v1.4.7 combined). Discipline failure on my part — when amending convention, I update the changelog/named-exceptions table but miss the lane-body text downstream.
...
7. **Scope namespace rule disambiguated** (DX new MED): v1.4.5-reused scopes stay unprefixed verbatim; new v1.4.7 scopes use `dailyos.<verb>.<noun>` prefix; manifest validation rejects unlisted aliases.
...
4. **W3-A new scope `read.workspace_source_provenance` → `dailyos.read.workspace_source_provenance`** per cycle 2 #8 namespace rule (new v1.4.7 scopes get the `dailyos.<verb>.<noun>` prefix; only v1.4.5-reused scopes stay unprefixed). Closes Eng MED.
...
Unlike v1.4.5 (which carried an approved project-level L0 verdict in the Linear description), v1.4.7's project description has no prior L0 approve. Cycle 1 of the L0 loop folds **project-scope review and wave-plan review into one pass** — codex Challenge / Eng / DX / CSO agents review both dimensions simultaneously against this packet. The packet is structured so each lane block names its project-scope rationale (mission-gate fit, scope-boundary justification) alongside its wave-scope contract (files owned, depends-on, done-when), so the same review cycle can clear both.

## Lessons folded in from v1.4.5 L0 (10 cycles to converge)

1. **Class-A propagation discipline.** When amending an API or relocating a contract, every active reference is updated in the same edit. v1.4.5 cycles 2/3/4/5 all hit "downstream propagation incomplete" findings; v1.4.7's draft pre-empts by grepping every named contract and its references in this initial draft.
2. **Shared types in W1-A `contracts.rs` from day one.** Trait wave-timing inversions cost v1.4.5 cycles 3-6. v1.4.7 W1-A pre-defines every cross-lane contract in `services/mcp_v2/contracts.rs` and pre-creates the full submodule skeleton with `pub mod` declarations so subsequent lanes never edit `mod.rs`.
3. **Frozen contract surfaces from cycle 1.** v1.4.5 spent cycles 8-10 on contract-sharpness amendments (DOS-474 idempotency key, DOS-489 query input, etc.). v1.4.7 pins typed input/output/error structs at cycle 1 to avoid the same back-and-forth.
4. **Migration slot block reserved upfront.** v220-v239 (after v1.4.5's v200-v219 + buffer). v1.4.7 likely needs few migrations because it's a contract layer over existing substrate.
5. **Cross-version dependency citations are explicit.** Every lane that consumes v1.4.5 substrate cites the exact frozen schema (`WorkspaceGraphProjection v1`, `PlaceDocumentInput v1`, etc.) rather than vague "uses v1.4.5 workspace memory."
6. **Issue-body cleanup at W0.** Several v1.4.7 Linear issues still reference the pre-renumbering `v1.4.9 / v1.4.10` numbering (these are now v1.4.5 / v1.4.7 respectively). W0 close gate includes a cleanup pass.
...
- v1.4.5 waves: `.docs/plans/v1.4.5-waves.md` — workspace memory substrate. **v1.4.7 implementation gates on v1.4.5 merge.** Specifically v1.4.7 consumes: `WorkspaceGraphProjection v1` schema (DOS-489 / W3-C), `PlaceDocumentInput / Receipt / Error` contract (DOS-474 / W4-C), `WorkspaceCategoryRegistry` + `WorkspaceCategory` enum (W1-A `contracts.rs`), `services::workspace_ingestion` mutation surface, `link::override_link` API (DOS-465 / W1-C), `pipeline::quarantine_source` API (W2-A), `WorkspaceFile { kind: WorkspaceFileKind }` `DataSource` variant (ADR-0107 amendment).
...
**Migration slots reserved:** v220–v239 (20 slots). v1.4.5 holds v200–v219; v180–v199 is v1.4.2 W5/W6 expansion buffer; v176–v179 is DOS-589; v175 is W4-E; v171–v174 is v1.4.2 W4-C.
...
## What v1.4.0–v1.4.5 leave behind (consumed contracts)

This section lists the EXACT frozen contracts v1.4.7 consumes. Citing them precisely is how we avoid v1.4.5 cycles 2-5's "downstream propagation incomplete" failure mode.
...
**From v1.4.5 (cited explicitly per consumer):**
...
- `DataSource::WorkspaceFile { kind: WorkspaceFileKind }` (ADR-0107 amendment merged in v1.4.5 W0; v1.4.7 surfaces this kind in graph projection responses).

**v1.4.7 does NOT ship:**
- New v1.4.5-style schema (workspace_ingestion is owned by v1.4.5; v1.4.7 only adds MCP-facing query / write tool layers).
- New trust UI primitives, claim-receipt UI, lint mode (those are v1.4.4 scope).
- Salience scoring / recommendations (v1.4.6 scope).
...
4. v1.4.5 wave plan — for the substrate v1.4.7 consumes; specifically the W3-C and W4-C lane sections that define the frozen schemas above.
...
| **L5 (Drift)** | W5 release close | `/plan-eng-review` + architect-reviewer against v1.4.7 DoD AND v1.4.5 substrate-consumer parity. |
...
| W3 — Workspace Memory Tools | (none expected) | All consume v1.4.5 contracts; no new schema |
...
1. **Issue-body renumbering cleanup.** Several v1.4.7 issues still reference the pre-renumbering `v1.4.9 / v1.4.10` taxonomy: DOS-167, DOS-168, DOS-170, DOS-171, DOS-172, DOS-173, DOS-175, DOS-479, DOS-480, DOS-481, DOS-482 all contain `v1.4.9` (now v1.4.5) and/or `v1.4.10` (now v1.4.7) references in their bodies. W0 includes a one-pass edit on each issue body to substitute current numbering. Doesn't change scope; eliminates a class of reviewer confusion.
...
4. **Scope naming convention frozen (cycle 2 amendment closes Challenge HIGH #5 + L0 question #3).** v1.4.7 adopts `<dot-namespace>.<verb>.<noun>` convention. v1.4.5's already-shipped `write.workspace_place_document` scope is kept verbatim — v1.4.7 W3-B's `place_document` MCP tool reuses it without rename. Scope-mapping table:

   | v1.4.5 scope (already shipped) | v1.4.7 reuse / extension |
   |---|---|
...
5. **Shared Primitive and Service Ownership doc** — confirm Linear doc `c729429f-79c9-4131-a243-5500d6e2a605` (originally for v1.4.5) extends to v1.4.7 every-tool-routes-through-services discipline. Likely already does; reconcile.
...
8. **Path-α maintenance issue protocol active.** Same as v1.4.5: theoretical hardening or scope-expansion findings during cycle 1+ codex review file as DailyOS Maintenance Linear issues (`b8e6aea4-d47e-4f3a-b03d-a05bec914aeb`), not blocking v1.4.7.
...
All v1.4.0–v1.4.5 invariants remain active. v1.4.7 adds:
...
| Every MCP write tool is on the explicit MCP write allowlist with operation-specific scope | Existing CI lint script `src-tauri/scripts/check_mcp_write_allowlist.sh` (extended in v1.4.5 W4-C) covers the new v1.4.7 write tools | W4 merge |
...
| Source-aware tool outputs (DOS-171, DOS-479) honor v1.4.5 sensitivity rendering (file paths absent unless caller has `read.entity_names`; raw claim text NEVER returned in aggregates) | Suite S test against the W3-C `WorkspaceGraphProjection v1` redaction rules | W3 merge |
...
- [ ] v1.4.5 cross-version dependency confirmation recorded (all v1.4.5 contracts listed in §"What v1.4.0–v1.4.5 leave behind" confirmed merged on `dev`)
...
- [ ] Path-α maintenance issues filed per the same DailyOS Maintenance project pattern v1.4.5 used (`b8e6aea4-d47e-4f3a-b03d-a05bec914aeb`) for any non-blocking findings surfaced during cycle 1+
...
- **Goal:** Build the MCP v2 gateway — the single entry point that receives MCP tool calls, validates `Actor::McpClient` policy, dispatches through the ability runtime, captures audit attribution, and emits signals. **W1-A pre-creates the entire `services/mcp_v2/` module skeleton** (per v1.4.5 lessons): `mod.rs` with all `pub mod` declarations, `contracts.rs` with shared types + traits + Null impls, empty placeholder files for every submodule that subsequent lanes fill.
- **Files owned (exclusive):**
  ...
  - Full submodule skeleton under `src-tauri/src/services/mcp_v2/` (cycle 2 amendment fixes module-tree mismatch):
    ...
    - `contracts.rs` (W1-A only owner) — defines (cycle 2 amendment makes the snippet compile-ready: renamed `when_NOT_to_call` to clippy-conforming `when_not_to_call` with serde rename for wire; defined `ParamSpec`/`ReturnSpec`/`ToolExample`/`ParamSchema`/`Side`; added shared `Page<T>` envelope; added `OpaqueConversationHandle`):
      ...
      // Scope namespace rule (cycle 3 DX disambiguation): scopes reused VERBATIM from v1.4.5 stay unprefixed
      ...
      // Shared pagination envelope (cycle 2 DX MED #5). v1.4.5 WorkspaceGraphResponse v1 is a documented pass-through exception.
      #[derive(Serialize, Deserialize)]
      ...
      // Typed errors propagated to MCP wire (cycle 4: ConversationRequired removed; gateway mints handles transparently per cycle 3 lifecycle first-write rule. Cycle-7 amendment: ExposureForbidden joins PairingRevoked / ConversationRevoked as a dedicated variant rather than sentinel-on-missing_scope.)
      pub enum ToolError {
          BadParams { detail: String },
          Unauthorized { missing_scope: Scope },     // scope-deficit only; auth-state revocations use dedicated variants
          PairingRevoked,
          ConversationRevoked,                       // also returned when cross-client handle binding violation per ADR-0102 §D.bis
          ExposureForbidden { tool_name: ScopedName }, // absent grant OR non-Invocable exposure per ADR-0102 §C.bis
  ...
  - **`OpaqueConversationHandle` lifecycle contract (cycle 3 DX MED — public wire shape):**
    ...
    - **Revocation:** server-side admin API can revoke a handle; subsequent presentations return the dedicated `ToolError::ConversationRevoked` variant and the caller must restart with a new conversation. (Cross-client binding violations per ADR-0102 §D.bis return the same variant.)
  ...
  - New `SignalType::McpToolInvoked` variant added to `signals/policy_registry.rs` (function-level addition; same pattern as v1.4.5 W3-B).
...
- **Depends on:** v1.4.5 fully merged; W0 ADR amendments merged.
...
- **Don't touch:** Other handlers; v1.4.5 graph projection (DOS-479 lane consumes that).
...
- **Done when:** Entity discovery tools support cursor pagination; ranked discovery returns top-N candidates ordered by recency × interaction × user-fit (existing v1.4.6-substrate scoring if present, else recency-only fallback); pagination cursor opaque + server-rotation-safe; `cargo test` + `cargo clippy -D warnings` clean.
...
- **Don't touch:** Other handlers; v1.4.6 substrate (consume read-only).
...
- **Goal:** Expose v1.4.5's `WorkspaceGraphProjection v1` through MCP tools so host models can search workspace memory by entity, category, lifecycle state, freshness, etc. Honors v1.4.5 sensitivity rules: file paths absent unless caller has `read.entity_names`; raw claim text NEVER in aggregates.
- **Files owned (exclusive):** `src-tauri/src/services/mcp_v2/handlers/tool_workspace_search.rs` (W1-A placeholder).
- **Don't touch:** v1.4.5 `services::workspace_ingestion::*` (consumer-only); placement handler (DOS-480 owns); resource handler (DOS-171 owns).
- **Depends on:** W1 + W2 merged; v1.4.5 W3-C (`WorkspaceGraphProjection v1`) frozen and merged.
- **Tools shipped (cycle 2 amendment renames to canonical `<dot>.<verb>.<noun>` per Class C):** `dailyos.search.workspace_memory(entity_filter?, category_filter?, lifecycle_filter?, freshness_filter?)` returning the v1.4.5 `WorkspaceGraphProjection v1` shape unchanged (pagination cursor passes through; this is the documented v1.4.5 pass-through exception to v1.4.7's shared `Page<T>` envelope). Plus `dailyos.read.workspace_source_provenance(file_id)` returning per-file provenance + ingestion run history + claim attribution. Required scopes: `read.workspace_graph` (reused verbatim from v1.4.5 W3-C) for `dailyos.search.workspace_memory`; `dailyos.read.workspace_source_provenance` (new v1.4.7 scope; cycle 4 amendment per Eng MED — new scopes carry the `dailyos.<verb>.<noun>` prefix per cycle 2 #8 namespace rule) for the provenance tool.
- **Security gate:** `/cso` at L0 plan AND L2 diff.
  - Sensitivity rendering: caller must have `read.entity_names` to see entity names in response; otherwise null per v1.4.5 redaction rule.
  - File paths NEVER returned (only `file_id`).
  - Raw claim text NEVER returned in aggregate responses (use `claim_summary` from v1.4.5 schema).
...
- **Goal:** Expose v1.4.5's `place_document` ability through an MCP write tool — `dailyos.write.place_document(entity_id, filename_hint, content_b64, category?, client_dedup_key?)` — returning v1.4.5 `PlaceDocumentReceipt v1` unchanged. Forwards to `place_document` ability with `Actor::McpClient` attribution. **Scope reuses v1.4.5's `write.workspace_place_document` verbatim** (cycle 2 Class C — no rename).
- **Files owned (exclusive):** `src-tauri/src/services/mcp_v2/handlers/tool_placement.rs` (W1-A placeholder).
- **Don't touch:** v1.4.5 `place_document` ability (consumer-only — handler is a thin wrapper); other handlers.
- **Depends on:** W3-A merged (establishes the W3 v1.4.5-substrate-consuming pattern); v1.4.5 W4-C frozen `PlaceDocumentInput / Receipt / Error` contract.
- **Security gate:** `/cso` at L0 plan AND L2 diff.
  - Caller must have `write.workspace_place_document` scope (cycle 2: reused verbatim from v1.4.5).
  - All v1.4.5 W4-C security rules apply: server-derived `source_asof`, transactional validation+write+enqueue, per-actor rate limits, idempotency by `(entity_id, content_sha256, client_dedup_key)`.
  - The `resolved_path` field in the receipt is gated by `read.entity_names` per v1.4.5 W4-C cycle-9 amendment — MCP handler honors this without modification.
...
- **Done when:** `dailyos.write.place_document` MCP tool (canonical naming per cycle 2 #8) delegates to v1.4.5's `place_document` ability; receipt serialization exact match for v1.4.5 `PlaceDocumentReceipt v1`; rate limits + idempotency tested; `/cso` L0 plan AND L2 diff approval recorded.
...
- **Goal:** MCP resources (browsable, URL-shaped) for entity + source profiles. URI scheme `dailyos://account/{handle}`, `dailyos://person/{handle}`, `dailyos://source/{handle}` where `{handle}` is a **server-minted opaque resource handle** (cycle 2 CSO MED #4 — never the underlying entity_id which could be enumerable/PII). Privacy-rendered per ADR-0108 + v1.4.5 sensitivity rules using the SAME redaction helper DOS-479 ships (cycle 2 CSO LOW #5 — DOS-171 must not implement parallel redaction logic; reuse the W3-A helper exactly).
- **Files owned (exclusive):** `src-tauri/src/services/mcp_v2/handlers/tool_resources.rs` (W1-A placeholder).
- **Don't touch:** Other handlers; v1.4.5 source registry (consumer-only).
...
- **Security gate:** `/cso` at L0 plan AND L2 diff.
  - Resource responses honor sensitivity rendering through the shared W3-A redaction helper (cycle 2 amendment): entity names null without `read.entity_names` scope; raw file paths NEVER returned; raw claim text NEVER returned (only `claim_summary` aggregates per v1.4.5 W3-C convention).
...
- L3 wave adversarial: codex-challenge + `/cso` + Suite S (sensitivity rendering across all three tools/resources; v1.4.5 contract pass-through fidelity).
- **Mandatory retro** (first MCP wave consuming v1.4.5 substrate; cross-version contract experience worth capturing).
...
- **Goal:** End-to-end validation of v1.4.7 MCP tools against real workspace data. Tools use DailyOS intelligence without bypassing abilities, source lifecycle, provenance, or privacy. v1.4.5 substrate consumed correctly.
...
- **Depends on:** W5-A merged (consumes the eval fixture corpus); v1.4.5 fully shipped (real workspace data dependency).
- **Validation axes:**
  ...
  4. **v1.4.5 contract fidelity:** tools that consume `WorkspaceGraphProjection v1` / `PlaceDocumentReceipt v1` / etc. produce schema-conforming responses.
...
- L5 drift check: integrated state matches v1.4.7 project DoD verbatim AND v1.4.5 contract pass-through verified.
- **Mandatory retro.**
...
### v1.4.5 contract pass-through
- [ ] dailyos.search.workspace_memory returns WorkspaceGraphProjection v1 unchanged
- [ ] dailyos.write.place_document delegates to v1.4.5 place_document ability with full PlaceDocumentReceipt v1 fidelity
- [ ] Sensitivity rendering matches v1.4.5 (file paths absent without read.entity_names; raw claim text absent in aggregates)
- [ ] Pagination cursor format matches v1.4.5 W3-C convention
...
| 3 | Scope naming convention | W0 (folded into ADR-0102) | **Resolved (cycle 2)** — `<dot-namespace>.<verb>.<noun>`; v1.4.5's `write.workspace_place_document` reused verbatim; full mapping table in §"Scope naming convention frozen" |
...
| 5 | Does `dailyos.read.portfolio_attention` consume an existing v1.4.6 salience ability or implement recency-only ranking? | W2-C (DOS-173) | **Open** — defer to W2-C L0 plan; v1.4.6 ship state determines |
...
| 7 | Does the v1.4.7 audit log re-use v1.4.5 audit substrate or introduce a separate MCP-specific audit table? | W1-A (DOS-168) | **Open** — defer to W1-A L0 plan with fresh grep against v1.4.5 audit code |
| 8 | Confirm v1.4.5 frozen contracts merged on dev before v1.4.7 W3 implementation starts | W3 | **Open** — coordination item; tracked at W0 close |
...
| v1.4.5-substrate-consuming lanes | DOS-171, DOS-479, DOS-480 (heaviest cross-version coupling) |
