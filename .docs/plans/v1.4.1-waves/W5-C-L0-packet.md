...
- **Channel 4 (composed get_entity_context children):** gate applied AT DOS-222's parent boundary via `src-tauri/src/services/claims.rs:3462` `prompt_input_sensitivity_allowed`, called from DOS-222's prompt-assembly site (DOS-222 implementation creates `src-tauri/abilities-runtime/src/abilities/detect_risk_shift/synthesis.rs::from_context` modeled on `src-tauri/abilities-runtime/src/abilities/prepare_meeting/synthesis.rs:381` `fn from_context`). See §"Channel 4 parent-boundary gate clarification" below for why DOS-222 re-gates independently of get_entity_context's own boundary.
- **Channel 5 (test seam):** `PrepareRiskShiftInput.context` carries `#[serde(default, skip_deserializing, skip_serializing)]` modeled on `src-tauri/abilities-runtime/src/abilities/prepare_meeting/synthesis.rs:45` (the cycle-4 pilot pattern); MCP/Agent cannot inject fabricated context JSON. DOS-222 implementation replicates this attribute on the corresponding input struct field.
- **Channel 6 (rendered prompt + canonical JSON):** gate applied at the canonical-inputs assembly point. Pilot reference: `src-tauri/abilities-runtime/src/abilities/prepare_meeting/synthesis.rs:2395-2408` and :2528-2532 show the canonical_inputs construction + `serde_json::to_string(&canonical_inputs)` serialization. DOS-222 implementation creates the same pattern at `src-tauri/abilities-runtime/src/abilities/detect_risk_shift/synthesis.rs` and calls `prompt_input_sensitivity_allowed` (`src/services/claims.rs:3462`) before canonical JSON serialization.
- **Channel 7 (template variables in `detect_risk_shift.v{n}.txt`):** gate applied at the `fn assemble` template-binding site. Pilot reference: `src-tauri/abilities-runtime/src/abilities/prepare_meeting/synthesis.rs:694` `fn assemble`. DOS-222 implementation creates the same `fn assemble` at `src-tauri/abilities-runtime/src/abilities/detect_risk_shift/synthesis.rs`; bindings derive from already-gated channel-6 inputs, and the template-substitution call site re-checks no unlisted `{{var}}` appears (prompt-fingerprint regression).
