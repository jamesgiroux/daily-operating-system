...
| I393 | Parent account detail page — portfolio surface (hotspots, cross-BU patterns, portfolio narrative) | Frontend / UX |
| I420 | Stakeholder–Person reconciliation — canonical name injection + post-enrichment fuzzy matching with confidence-tiered linking | Backend / Intelligence + Frontend / Entity |
...
- New integrations
- REDACTED or CRM sync (separate concern; entity hierarchy can be populated manually for now)
...
### I393 — Parent account detail page
...
### I420 — Stakeholder–Person reconciliation

1. The enrichment prompt for any account with linked Person entities includes a "Known Contacts" section listing each linked person's canonical name, role, and email. Verify by triggering enrichment on an account with 3+ linked people and inspecting the prompt.
2. After enrichment, stakeholder entries for linked people use the exact canonical name from the Person entity, not an abbreviation or nickname. Verify with a person whose meeting-note name differs from their entity name.
3. A stakeholder insight that matches a linked Person entity at ≥ 0.85 confidence has a `personId` field in `intelligence.json`. Verify: at least one stakeholder on an account with linked people has `personId` set.
4. A stakeholder insight matching at 0.6–0.85 confidence has a `suggestedPersonId` field. The frontend renders a confirmation prompt. Clicking "Yes" promotes the suggestion to a confirmed `personId`.
5. A stakeholder with no match (< 0.6) has neither field. The existing "Create contact" hover action still works.
6. The `StakeholderGallery` uses `personId` (when present) for deterministic matching instead of name comparison. Larkspur ring and link icon render from `personId`. Name-based matching remains as fallback for older intelligence.json files.
7. Manually editing a stakeholder's name, then triggering re-enrichment, preserves the edited name. Reconciliation does not overwrite user edits.
8. An account with a linked "John Smith" does not auto-link an unrelated "John S." stakeholder who has a different role and appears in different meetings.
...
I382 (partner entity type)
  ...
  └── informs I384/I393 (partner accounts should not render portfolio sections
...
I393 (parent detail page)
  └── addBlockedBy I384 — page renders from portfolio section of intelligence.json;
       that section doesn't exist until I384 ships

I420 (stakeholder–person reconciliation)
  └── independent — extends enrichment pipeline and StakeholderInsight type;
       no other v0.13.3 issues block it
```
...
5. I393 (parent detail page) — the frontend surface, renders I384 output
6. I420 (stakeholder reconciliation) — enrichment pipeline enhancement, independent of 1–5

---

## Ship criteria

All acceptance criteria above are met across all 6 issues. A real parent account with 3+ children renders a Portfolio chapter with a synthesized health summary, at least one hotspot if any child has a risk signal, and cross-BU patterns if applicable. BU signals propagate to the parent without user action. A high-confidence parent signal fans out to all direct children within one propagation cycle. Partner accounts render with a distinct badge. The AccountsPage shows three groups. Stakeholder entries for known Person entities show deterministic links via `personId` after enrichment. No regressions on leaf-node account, person, or project pages.
...
6. Open an account detail page with 3+ linked Person entities. Confirm: stakeholders in "The Room" who match linked people show a larkspur avatar ring and link icon. At least one stakeholder has `personId` set in `intelligence.json` (not just a UI-side name match). An unlinked stakeholder shows no ring and shows "Create contact" on hover.
