...
| I394 | Week page past meeting duration shows NaN — guard against invalid date strings in formatDurationFromIso | Frontend / Bug |
| I395 | Email relevance scoring — signal-driven surfacing for briefings using entity linkage, Bayesian fusion, embedding relevance, keyword matching | Backend / Intelligence + Frontend / UX |
...
5. The query is `in:inbox` only — no `newer_than` window. All inbox emails are fetched regardless of age. Verify emails older than 3 days still appear if they're in the inbox.
...
2. The `emails` table marks rows for the 3 vanished emails with `resolved_at` timestamp (the new `emails` table subsumes the old `email_threads` table).
...
3. The poll cycle makes a separate sent mail query (`in:sent`) to detect user replies — not just incoming mail. Verify via logs showing a distinct sent-mail API call each cycle.
...
1. Open the Weekly Forecast page. Past meetings display a valid duration (e.g., "30m", "1h") — never "NaNm" or "NaN".
2. `formatDurationFromIso` in `src/lib/meeting-time.ts` guards against `NaN` from `getTime()` before performing arithmetic. If either timestamp produces an invalid Date, the function returns `null` (no duration shown) instead of `"NaNm"`.
3. Meetings with missing `endTime` (null/undefined) still show no duration — not a regression from the NaN fix.

---

### I395 — Email relevance scoring

1. Scoring function exists and produces real scores. `SELECT email_id, relevance_score FROM emails WHERE relevance_score IS NOT NULL` returns rows. Scores range 0.0–1.0. Emails from known contacts at known accounts score higher than unknown senders. Calendar notifications score below 0.10.
2. Daily briefing shows scored emails, not mechanical "replies needed." Section labeled "Worth Your Attention." Top 3 by score, threshold 0.15. Each shows contextual summary + entity name. No calendar notifications, no noreply addresses, no newsletters.
3. Email page sorted by relevance score. First email = highest scored. Renewal discussion from key account above internal newsletter. Score determines order, not mechanical priority.
4. Entity linkage affects score. Email from `jack@acme.com` where Acme is tracked with active signals scores higher than `random@unknown.com`.
5. Meeting relevance affects score. Emails about entities with meetings today score higher than emails about entities with no meetings today.
6. Noise emails score near zero. Calendar notifications, noreply senders, comment-reply addresses below 0.10. Appear at bottom of email page, never on briefing.
7. Scoring runs as part of enrichment pipeline. After enrichment, scores computed and persisted to `emails.relevance_score`. No manual trigger.
8. Score rationale is human-readable. `score_reason` field explains scoring. Example: "Acme account + renewal keyword + meeting today + high urgency."
...
I395 (relevance scoring)
  └── depends on I367 (enrichment output), I368 (DB persistence), I372 (signal compounding)
  └── uses: signals/fusion.rs, signals/relevance.rs, signals/decay.rs, focus_prioritization.rs patterns
...
I394 (past meeting NaN duration) — independent, frontend bug fix
```
...
All acceptance criteria above are met across all 14 issues. No P0 or P1 bugs open. The email page shows contextually synthesized intelligence for every email — not raw sender+subject rows. An email from a known contact at a known account produces a summary that references the entity's current state and upcoming meetings. Emails archived in Gmail do not appear in DailyOS. "Replies Needed" accurately reflects threads awaiting user response. The meeting detail page shows email context from attendees. Email signals compound with entity intelligence.
