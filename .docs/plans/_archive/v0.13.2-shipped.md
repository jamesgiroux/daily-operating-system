...
4. `post_meeting.rs` fires reliably after a meeting ends. Verify with real data: after a completed meeting, `SELECT signal_type, entity_type, created_at FROM signal_events WHERE signal_type = 'post_meeting_followup' ORDER BY created_at DESC LIMIT 5` returns rows with timestamps matching the meeting end time. (Note: post-meeting signals use `signal_type = 'post_meeting_followup'`, not a `source` filter — filtering by `source LIKE '%post_meeting%'` would return nothing as the source field records the entity origin, not the pipeline stage.)
...
- Automation failure (signal chain broke, pipeline got stuck — see: Crestview Media Auto this morning)
