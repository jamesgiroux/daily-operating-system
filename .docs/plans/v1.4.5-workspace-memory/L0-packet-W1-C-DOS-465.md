...
- `src-tauri/src/services/workspace_ingestion/link.rs` — fills the W1-A-pre-created placeholder:
  ...
  - `LinkRepo::add_link(file_id: &str, entity_type: crate::entity::EntityType, entity_id: &str, attribution_source: LinkAttributionSource, confidence: f64, rationale: Option<&str>, actor: &str) -> Result<DocumentEntityLinkId, LinkError>` — **V1.3 fold #1 tombstone guard wraps SELECT + INSERT in `BEGIN IMMEDIATE` transaction** (closes the V1.2 pre-SELECT-then-INSERT race a concurrent `reject_link` could exploit). Classifier/Backfill/DriveMetadata sources: open `BEGIN IMMEDIATE` → pre-check for `rejected = 1` row → if present, `ROLLBACK` + return `LinkError::Tombstoned`; else `INSERT … ON CONFLICT (file_id, entity_type, entity_id) WHERE rejected = 0 DO NOTHING RETURNING link_id` → `COMMIT`. Entity-intake/user-relink/MCP/frontmatter sources bypass the rejected-row check but still wrap their INSERT in a transaction for atomicity. Fallback `SELECT link_id WHERE …` when RETURNING returns zero rows.
...
- **`RunsRepo::start_run + complete_run + find_by_idempotency_key`:**
  ...
  - **V1.3 fold #3 + #6:** Repeat `start_run(seed)` with same `(file_id, content_sha256, mode)` after first success → returns `Err(RunsError::AlreadyCompleted { existing: ExistingRunReceipt })` directly (service-layer guard, NOT generic DbError). Test asserts total `status='success'` rows = 1.
