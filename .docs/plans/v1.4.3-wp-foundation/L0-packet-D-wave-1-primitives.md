...
- **V1.4 (2026-05-18) — lock candidate.** Folded L0 cycle 4 validate-closures findings. Cycle 4 verdicts: code-reviewer APPROVE (all V1.3 closures verified); design-reviewer APPROVE (all V1.3 closures verified; flagged one V1.0 carry-over path-α — StatusDot semantic-axis discrepancy); codex consult CONDITIONAL (commit 2 chrome tests must not depend on D2-D4 block dirs; workflow gate must skip absent block dirs); codex challenge A CONDITIONAL (§5.8 loading-state needs concrete W2 sentinel); codex challenge B APPROVE (HealthBadge label-discipline v1.4.4 deferral correct). 2 APPROVE + 2 CONDITIONAL strictly surgical + 0 BLOCK + 0 class-pattern active = lock-ready after V1.4 fold.
  - **§5.8 chrome service test fixture surgical fold.** (a) `chrome_service_integration_fixture.rs` uses **mock primitive fixtures the test owns**, builds 4×11=44 in-test compositions, does NOT depend on per-primitive `wp/dailyos/blocks/<slug>/` outputs from PR-D2/D3/D4. (b) Loading state for W2 = empty `claim_refs` + unresolved/pending projection; `render_hints.chrome_state=Loading` NOT required (deferred to v1.4.4 W4 per producer-vs-surface boundary). (c) Workflow grep gate scoped to existing `render-functions.php` files only (`find ... -print0 | xargs -0 grep -L`); does not fail PR-D1 for absent block dirs.
  - **Path-α filed to maintenance project `b8e6aea4-d47e-4f3a-b03d-a05bec914aeb` (V1.4 — cycle 4 design-reviewer carry-over flag):** StatusDot semantic-axis discrepancy — §7.1 Axis 1 lists `healthy/warning/critical/unknown` but `StatusDot.md:9` declares `connected/disconnected/loading/error`. V1.0 carry-over, not V1.3 regression; file to maintenance + true up in v1.4.4 W2 surface-migration work where StatusDot composes into surfaces. Does NOT block V1.4 lock.
  - **Packet locks at V1.4.** Cycle 5 not dispatched — V1.4 changes are strictly surgical (mock fixtures, loading-state sentinel, grep gate scoping) with no new class-pattern. Per `feedback_review_loop_l6_policy.md` convergence rule: 2 APPROVE + 2 surgical CONDITIONAL all folded = unanimous-equivalent. W2 implementation can proceed on the basis spec'd here per §15.
...
- **Rust harness extension** — `src-tauri/abilities-runtime/tests/chrome_service_integration_fixture.rs` (new). Drives the harness with four fixture compositions per primitive using **mock primitive fixtures the test owns** — NOT real per-primitive `wp/dailyos/blocks/<slug>/` outputs (which don't exist until PR-D2/D3/D4 land). The fixture builds 4×11 = 44 in-test compositions (4 state branches × 11 primitives) and asserts the rendered HTML contains the corresponding chrome partial marker (e.g., `data-chrome="empty"` set by `render-empty.php`) and does NOT contain the marker for any other state. The 4 state branches (V1.4 — concrete W2 representation per cycle 4 challenge Shape A):
  - (a) **Ready state**: full payload, non-empty `claim_refs`, resolved projection.
  - (b) **Loading state**: empty `claim_refs` + unresolved/pending projection result. **For W2, `render_hints.chrome_state=Loading` is NOT required** — the surface derives loading state from the projection's unresolved/pending status. Producer-side `render_hints.chrome_state` adoption is deferred to v1.4.4 W4 per §5.8 producer-vs-surface boundary.
  - (c) **Empty state**: empty `claim_refs` + resolved-to-no-data projection.
  - (d) **Error state**: projection error.
- **WP-side standalone test** — `wp/dailyos/tests/blocks/chrome/ChromeServiceTest.php` (new). PHPUnit test that invokes each `render-(empty|loading|error).php` partial directly with a minimal fixture context the test owns (NOT dependent on per-primitive block outputs from PR-D2/D3/D4), asserts the rendered HTML matches a snapshot, and verifies no inline `style=` attribute appears in the output (per memory `feedback_no_inline_css.md`).
- **Workflow gate wiring (V1.3 fixes the absent gate cycle 3 challenge #3 caught; V1.4 scopes the grep gate)** — `.github/workflows/block-kit-integration.yml` adds a new step: "Chrome service coverage" that runs `cargo test -p abilities-runtime --test chrome_service_integration_fixture` AND `pnpm phpunit wp/dailyos/tests/blocks/chrome/` AND a grep gate that operates **only over `wp/dailyos/blocks/<slug>/render-functions.php` files that currently exist on the branch**. V1.4 explicit: the grep gate uses `find wp/dailyos/blocks -name render-functions.php -print0 | xargs -0 grep -L 'require_once.*_shared/chrome/render-\(empty\|loading\|error\)\.php'` and fails only if an EXISTING file is missing the `require_once`. PR-D1 has zero block dirs in `wp/dailyos/blocks/<slug>/` until PR-D2/D3/D4 add them; the gate must not fail PR-D1 for absent dirs (cycle 4 codex consult condition).
...
**LOCKED at V1.4 (2026-05-18).** Cycle 4 returned 2 APPROVE (code-reviewer + design-reviewer) + 2 strictly surgical CONDITIONAL (codex consult + codex challenge A) + 1 APPROVE (codex challenge B = HealthBadge deferral) + 0 BLOCK. V1.4 folded the 2 CONDITIONALs as surgical patches to §5.8; no class-pattern active; no V1.3 regression introduced. Per `feedback_review_loop_l6_policy.md` convergence rule, lock authorized; W2 implementation may proceed.
