...
The second surface is accidental live evaluation. The harness must be structurally unable to hit PTY, HTTP, Google/Glean/Slack/Gmail/REDACTED, or a live DB. Missing replay data is a typed harness failure, matching `ProviderError::ReplayFixtureMissing` (`src-tauri/src/intelligence/provider.rs:183-190`, `:290-295`), not a fallback to Live. Capture mode is opt-in local tooling, emits no secrets, and writes `fixture_identity_map.json` outside the repo.
