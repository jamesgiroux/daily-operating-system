# Codex Agent Briefing — v1.4.0 Wave 4 L0 Plan Drafts
...
- Wave 2 (DOS-209 + DOS-259) is merged. Wave 3 (8 agents) is in flight in parallel sessions — its L0 plans are at `.docs/plans/wave-W3/`. Wave 4 pre-work is now in progress.

## Your deliverable

A single file: `.docs/plans/wave-W4/<DOS-NNN>-plan.md` matching the 10-section template at `.docs/plans/v1.4.0-waves.md` §"Plan-review template" (lines ~87-133 of that file).
...
4. **Read the W3 L0 plans** for your upstream dependencies under `.docs/plans/wave-W3/` — these contain frozen contracts your plan must build on.
5. **Grep current-code surfaces** named by the ticket — every "files to create/modify" claim cites a real `file:line`. Use `rg` from the repo root.
6. **Read the W2 reference plan** for tone/depth: `.docs/plans/wave-W2/DOS-259-plan.md`.
7. **Write your plan file.**

## Hard constraints

- Write **exactly one file**: `.docs/plans/wave-W4/<DOS-NNN>-plan.md`. Nothing else.
