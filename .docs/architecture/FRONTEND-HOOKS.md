...
| `useAppLock` | `useAppLock.ts` | 21 | get_lock_status | app-locked, app-unlocked |
| `useAppState` | `useAppState.ts` | 132 | clear_demo_data, get_app_state, install_demo_data, set_tour_completed, set_wizard_completed | — |
| `useBackgroundStatus` | `useBackgroundStatus.ts` | 100 | — | background-work-status |
...
| `useGoogleAuth` | `useGoogleAuth.ts` | 113 | disconnect_google, get_google_auth_status, start_google_auth | google-auth-changed, google-auth-failed |
| `useInbox` | `useInbox.ts` | 101 | get_inbox_files | inbox-updated |
| `useIntelligenceCorrection` | `useIntelligenceCorrection.ts` | 139 | submit_intelligence_correction | — |
| `useIntelligenceFeedback` | `useIntelligenceFeedback.ts` | 109 | get_entity_feedback, submit_intelligence_correction | — |
...
| `useMe` | `useMe.ts` | 125 | create_user_context_entry, delete_user_context_entry, get_user_context_entries, get_user_entity, update_user_context_entry, update_user_entity_field | user-entity-updated |
| `useMeetingOutcomes` | `useMeetingOutcomes.ts` | 53 | — | — |
| `useNotifications` | `useNotifications.ts` | 161 | refresh_emails | email-error, enrichment-glean-degraded, enrichment-glean-fallback, google-auth-changed, operation-delivered, system-status |
...
| `useWorkflow` | `useWorkflow.ts` | 230 | get_execution_history, get_next_run_time, get_workflow_status, run_workflow | — |
...
- `get_entity_suppressions` (1 hooks)
- `get_entity_metadata` (1 hooks)
...
- `confirm_pending_stakeholder` (1 hooks)
...
- `intelligence-updated` (2 hooks)
- `inbox-updated` (2 hooks)
- `google-auth-changed` (2 hooks)
- `background-work-status` (2 hooks)
- `user-entity-updated` (1 hooks)
- `transcript-processed` (1 hooks)
- `system-status` (1 hooks)
- `preset-changed` (1 hooks)
- `prep-ready` (1 hooks)
- `post-meeting-prompt-fallback` (1 hooks)
- `post-meeting-prompt` (1 hooks)
- `operation-delivered` (1 hooks)
- `google-auth-failed` (1 hooks)
- `glean-auth-failed` (1 hooks)
- `glean-auth-changed` (1 hooks)
- `entity-updated` (1 hooks)
- `enrichment-progress` (1 hooks)
- `enrichment-glean-fallback` (1 hooks)
- `enrichment-glean-degraded` (1 hooks)
- `enrichment-complete` (1 hooks)
- `emails-updated` (1 hooks)
- `email-error` (1 hooks)
- `content-changed` (1 hooks)
