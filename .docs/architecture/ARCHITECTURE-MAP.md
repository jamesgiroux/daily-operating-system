...
> **SUPERSEDED 2026-03-02.** Replaced by `.docs/architecture/MODULE-MAP.md`. Retained for historical reference (documents the v0.13.2 extraction decisions).

**Purpose:** Structural reference for "what exists where." Facts, not analysis.
**Last verified:** 2026-02-22 (updated for v0.13.2)
...
src-tauri/src/
  ...
  commands.rs .......... ~6,992 lines   IPC handlers (260+ commands, thin wrappers for extracted domains)
  ...
  db/ .................. Data access layer (domain-split, v0.13.2)
    mod.rs ............. ActionDb struct, connection, helpers (441 lines)
    mod_tests.rs ....... Extracted test module (3,526 lines)
    ...
    emails.rs .......... Email signal queries

  services/ ............ Service layer (Phase 2 complete, v0.13.6 — 11 files, 6,574 lines)
    mod.rs
    entities.rs ........ Entity signal prose formatting (315 lines)
    dashboard.rs ....... Dashboard result assembly (902 lines)
    actions.rs ......... Action CRUD, status transitions, signals (466 lines)
    accounts.rs ........ Account lifecycle, team, events, merge, internal org, lists (1,083 lines)
    people.rs .......... Person CRUD, merge, delete, entity linking (383 lines)
    meetings.rs ........ Intelligence assembly, prep loading, entity linking,
                         attendee hydration, outcomes, transcript, refresh (1,517 lines)
    projects.rs ........ Project CRUD, list, notes, bulk create, archive (461 lines)
    emails.rs .......... Email enrichment, entity emails, dismissals, archive (624 lines)
    settings.rs ........ Config mutations: mode, workspace, AI, hygiene, schedule (303 lines)
    intelligence.rs .... Entity enrichment, field edits, stakeholders, risk (242 lines)
    integrations.rs .... Claude Desktop, Quill, connector status (267 lines)
  ...
  intelligence/ ........ Intelligence computation (refactored from monolithic files)
    ...
    lifecycle.rs ....... Meeting intelligence lifecycle — mechanical quality assessment
                         + MeetingPrepQueue enqueue (no PTY calls, ADR-0086 v0.13.2)
    prompts.rs ......... AI prompt building (dead enrich_entity_intelligence removed v0.13.2)
  ...
  Other modules:
    ...
    intel_queue.rs ..... Background intelligence enrichment queue (entity-level AI via PTY)
    meeting_prep_queue.rs Background mechanical prep assembly (no AI, ADR-0086)
                         Startup sweep enqueues all future meetings with entities but no prep.
                         Processes: gather_meeting_context → build_prep_json → prep_frozen_json.
...
src/
  ...
  types/index.ts .......... ~1,590 lines (v0.13.2: removed dead valueDelivered/ValueItem)
...
### Daily Pipeline (Prepare → Deliver)
...
```
              ...
              └─────────────┬───────────────────────────┘
                            │
                   commands.rs → services/*.rs (IPC dispatch)
                            │
                            ▼
              ┌─────────────────────────────────────────┐
              │           React Frontend                 │
              │  hooks/ → invoke() → services/*.rs       │
              │  Tauri events → real-time updates        │
              │  design-tokens.css → editorial styling   │
              └─────────────────────────────────────────┘
```

### ADR-0086 Intelligence Flow (v0.13.2)

Meeting prep is a mechanical consumer of entity intelligence. No AI at the meeting level.

```
  Signals (email, calendar, user edits, transcripts)
      │
      ▼ emit_signal_and_propagate()
  ┌──────────────────────────────────┐
  │  Signal Bus + Propagation Engine  │
  │  rules.rs → derived signals       │
  │  invalidation.rs → clear stale    │
  │    meeting preps                  │
  └──────────────┬───────────────────┘
                 │
                 ▼
  ┌──────────────────────────────────┐
  │  Intel Queue (entity-level AI)    │
  │  PTY → Claude Code → intelligence │
  │  .json per entity (account,       │
  │  project, person)                 │
  └──────────────┬───────────────────┘
                 │
                 ▼
  ┌──────────────────────────────────┐
  │  MeetingPrepQueue (mechanical)    │
  │  Startup sweep: all future mtgs   │
  │  with linked entities + no prep   │
  │                                   │
  │  gather_meeting_context_single()  │
  │  → inject_entity_intelligence()   │
  │  → build_prep_json()              │
  │  → prep_frozen_json in DB         │
  │                                   │
  │  Triggers: startup sweep, entity  │
  │  relink, signal invalidation,     │
  │  calendar upsert                  │
  └──────────────┬───────────────────┘
                 │
                 ▼
  ...
  │  load_meeting_prep_from_sources   │
  │  Priority: prep_frozen_json →     │
  │  disk file → prep_context_json    │
  └──────────────────────────────────┘
```
...
```
  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌──────────────┐
  │ intel_queue │  │ signals/   │  │ processor/ │  │ hygiene    │  │ meeting_prep │
  │ (entity AI) │  │ (bus+prop) │  │ (inbox AI) │  │ (data QA)  │  │ _queue (mech)│
  └──────┬─────┘  └──────┬─────┘  └──────┬─────┘  └──────┬─────┘  └──────┬───────┘
         └───────────────┴───────────────┴───────────────┴───────────────┘
                              │
                        writes to DB
...
| 8 | Intelligence processor | `intel_queue::run_intel_processor()` | Background AI enrichment for entities (PTY) |
| 8b | Meeting prep processor | `meeting_prep_queue::run_meeting_prep_processor()` | Mechanical prep assembly from entity intelligence (no AI). Startup sweep enqueues all future meetings with entities but no prep. |
...
**Call pattern (post v0.13.2):**
```
Frontend hook (useX.ts) → invoke("command_name", args) → commands.rs handler → services/*.rs → db/ methods
```
For extracted domains (actions, accounts, people, meetings), the handler is a thin wrapper. For remaining domains, business logic is still in the handler.
...
1. **Emission** (`bus.rs`): Events are appended to `signal_events` table with entity_id, signal_type, confidence score, source, and payload. Also flags linked meetings with `has_new_signals = 1`.
2. **Propagation** (`propagation.rs` + `rules.rs`): Cross-entity derivation rules fire when signals arrive — e.g., a person signal propagates to their accounts. v0.13.2: all user-edit and email-bridge signals now use `emit_signal_and_propagate()` so propagation fires on every user action.
...
6. **Invalidation** (`invalidation.rs`): Signal changes trigger prep regeneration for affected meetings. v0.13.2: added `stakeholders_updated`, `team_member_added`, `team_member_removed` to invalidation triggers.

**Propagation rules (v0.13.2 audit):**
- `rule_person_job_change` — listens for `title_change`, `company_change` (emitter: Clay)
- `rule_overdue_actions` — listens for `proactive_action_cluster` (emitter: proactive engine)
- `rule_champion_sentiment` — listens for `negative_sentiment` (emitter: executor)
- `rule_departure_renewal` — listens for `company_change` (emitter: Clay). Note: `person_departed` signal has no emitter (partial dead path).
- `rule_renewal_engagement_compound` — listens for `renewal_proximity` (emitter: proactive engine)
- ~~`rule_meeting_frequency_drop`~~ — removed v0.13.2 (no emitter existed)
