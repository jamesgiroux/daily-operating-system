...
| --- | --- |
| Server-owned local MCP identity | `src-tauri/src/services/mcp_v2/local_runtime.rs` creates/persists the local stdio client id in macOS Keychain through the pinned `/usr/bin/security` helper; `src-tauri/src/mcp/main.rs` no longer reads `DAILYOS_MCP_CLIENT_ID`. |
...
| Final MCP response envelope preserves typed output | `unwrap_response` returns JSON text with `dailyos.conversationHandle` plus `result`; transport tests assert trust, sensitivity, and provenance survive the wrapper and that `_dailyos` cannot appear in typed results. ADR-0137 names this as the v1.4.9 W2 local-stdio wire migration boundary from the earlier raw top-level JSON text result. |
...
| `cargo test --manifest-path src-tauri/Cargo.toml services::mcp_v2 --lib` | Pass - 111 tests |
| `cargo test --manifest-path src-tauri/Cargo.toml services::mcp_v2::local_runtime --lib` | Pass - 12 tests covering Keychain first-seed race handling, pinned `/usr/bin/security` helper path, Keychain timeout, 24-hour conversation remint, revoked-handle, and concurrent-store behavior |
