...
**Why:** The current classifier correctly identifies newsletters (low) and customer/urgent emails (high via domain or keyword). But it's context-blind — it doesn't know that Beacon Motors has a renewal approaching, risks flagged, and active intelligence about the renewal conversation. An email with "order form" + Beacon Motors should be high priority not because "order form" is a keyword, but because the system already knows Vintage's renewal is the most important thing happening right now. The email's importance is contextual, not textual.
...
- Query active signals for Vintage → `renewal_approaching` (0.9), `engagement_warning` (0.7), `risk_flagged` (0.8)
