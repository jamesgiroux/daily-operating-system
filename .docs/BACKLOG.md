...
- "REDACTED Digital Experience" (CSV) → "REDACTED - Digital Experience" (entity)
...
**Status:** Decomposed. The original I143 described a renewal management product (kanban pipelines, ARR waterfall charts, health score dashboards, a dedicated `/renewals` page). That's Gainsight, not DailyOS. The chief of staff tells you "Crestview Media Auto renews in 6 weeks and the relationship is cooling" — they don't hand you a renewal pipeline kanban.
...
- Surfaced on the daily briefing when compound confidence exceeds threshold — the chief of staff says "Crestview Media Auto renews in 6 weeks and you haven't met with them since January."
...
A user demos "Agentforce" (a project) to Jefferson Advisors (a customer). The system can't connect the meeting to the Agentforce project because project matching is entirely manual. It resolves to "REDACTED" (the parent company) via domain matching and pulls REDACTED Security intelligence instead of Agentforce context. The user corrects it, but the next briefing re-run doesn't re-trigger enrichment with the correct entity's intelligence. This pattern — wrong entity, stale intelligence, manual correction that doesn't cascade — erodes trust in the "your day is ready" promise.
...
2. **Project keyword matching** — Compare meeting title + description against project names, aliases, and configured keywords. "Agentforce Demo with Jefferson Advisors" matches project "Agentforce" on keyword.
...
- **Low confidence (<0.5):** Don't auto-link. Surface as proactive hygiene suggestion: "Untagged meeting: Agentforce Demo with Jefferson Advisors — did you mean Agentforce project?"
- **Conflicting signals:** Surface as hygiene item with the competing options: "REDACTED vs Agentforce for this meeting — attendees suggest REDACTED, title suggests Agentforce."
...
**The Beacon Motors example:** User removes internal discussion items from customer agenda → system records `source_context=internal_meeting` items were rejected for `prep_type=customer_meeting` → next time, internal-sourced items have lower weight in customer-facing prep for this account → over time, generalizes to all accounts.
...
- Future: MCP integrations, Gong, REDACTED, Linear, etc.
...
- CRM integration: sync email signals to REDACTED/Gainsight
...
- "Nexus Analytics renewal is in 30 days with no QBR scheduled and the last executive contact was 45 days ago" (temporal + gap correlation)
...
- "Linked 3 orphaned meetings to Nexus Analytics account" (meeting linking)
- "Reclassified <james@company.com> as internal" (relationship fix)
- "Rolled over Nexus Analytics renewal: 2025-12-31 → 2026-12-31" (renewal rollover)
...
4. **Intelligence confidence scoring**
   - "Nexus Analytics intelligence is based on 2 source files. Acme has 14."
...
**What Glean is:** Enterprise AI search platform that indexes 100+ SaaS apps (Google Workspace, Slack, REDACTED, Confluence, etc.) and provides permissions-aware search, a chat assistant, and agents on top of a unified knowledge graph. Our business has acquired it but not yet implemented.
...
DailyOS is limited to what one person can see: their calendar, email, workspace files. Glean unlocks **institutional memory** — the Confluence page a colleague wrote about an account's architecture, the Slack thread where engineering discussed a customer's feature request, the REDACTED notes from the AE.
...
**Motivation:** Internal organizations (ADR-0070) require depth. A user at Automattic might need: Automattic → VIP Division → Sales Org → CS Group → Key Accounts → Corporate Key Accounts. External enterprise accounts also benefit: Crestview Media → Diversification → Regional → Local. The current one-level limit forces flat representations of inherently hierarchical structures.
...
**Why:** The current classifier correctly identifies newsletters (low) and customer/urgent emails (high via domain or keyword). But it's context-blind — it doesn't know that Beacon Motors has a renewal approaching, risks flagged, and active intelligence about the renewal conversation. An email with "order form" + Beacon Motors should be high priority not because "order form" is a keyword, but because the system already knows REDACTED's renewal is the most important thing happening right now. The email's importance is contextual, not textual.
...
- Resolve email to entity → Beacon Motors
- Query active signals for REDACTED → `renewal_approaching` (0.9), `engagement_warning` (0.7), `risk_flagged` (0.8)
- Compound: medium email + high-signal entity = high priority
- Surface with context: "Renan sent the renewal order form to Beacon Motors — renewal approaching in 47 days"
