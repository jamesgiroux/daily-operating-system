...
The end-of-RSM slide needs a number on it. Two numbers and a short list.
...
- `use_with_caution` in the 50-75% range. Genuinely means "go verify."
- `needs_verification` < 40%. If it's confident here, we've mis-calibrated.

One chart. If it holds, everything else in the product is credible. If it doesn't, nothing else matters.

**Retrieval benchmark (BrainBench-style, [DOS-261](https://linear.app/a8c/issue/DOS-261) permitting):** Precision@5 and Recall@5 on a fixed corpus of real queries, before and after a mid-month tuning pass. Plus graph-only vs hybrid F1 ablation once claim edges are populated. GBrain saw 86.6% vs 57.8%; I'd like a comparable lift.

**The rest, easy to track:**

- Ghost-resurrection incidents. Target: 0. Counted, instrumented.
- Learning one-pagers written. Target: 4 of the 6 stubbed.
- Cross-team conversations with a named next step. Target: at least 1.
