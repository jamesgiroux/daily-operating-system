...
3. **The Jefferies/Adobe Fonts test:** Open the running app. For the Jefferies meeting (entity: Agentforce project), the coherence check should detect that the intelligence text mentioning "Adobe Fonts" is semantically distant from meeting titles like "Agentforce", "Salesforce", "VIP". Verify: `SELECT coherence_score FROM entity_intel WHERE entity_id = '<agentforce_id>'` returns a score below threshold. After re-enrichment runs, the intelligence no longer mentions Adobe Fonts.
