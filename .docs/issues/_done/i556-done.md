...
12. Book of Business report reads `entity_assessment.org_health_json` for Glean-enriched accounts. `resolve_health_band()` uses REDACTED health band when available (more reliable than computed-only). Verify: BoB snapshot table for a Glean-enriched account shows REDACTED-sourced health band, not just local `entity_quality.health_score`.
