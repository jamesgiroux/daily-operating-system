...
1. **REDACTED account records** (doc_type: `REDACTED_account`): Snippets contain structured field-value pairs like `Health Score: Green`, `Renewal Likelihood: Yellow`, `Growth Tier: Tier 1 - Expansion`.
...
1. REDACTED account records (most authoritative — source of truth for commercial data)
...
The `gathered_at` timestamp records when the Glean data was parsed. The `source` field records which Glean document type provided the data (e.g., `"glean_REDACTED"`, `"glean_zendesk"`).
...
5. The parser correctly identifies `REDACTED_account` doc_type results as highest priority source
...
- Structured extraction of non-health Glean fields (REDACTED contacts, Gong recordings, etc.) — future issue
