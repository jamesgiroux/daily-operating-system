...
These return broad, unfocused results. When tested against a real Glean instance (Blackstone account), the generic `"{name} account"` query returned a mix of Salesforce data, commercials, key roles, support signals, and account activity — but Glean itself suggested more targeted queries would yield better results.
...
Glean queries are shaped by the active role preset's vocabulary, vitals, lifecycle events, and briefing emphasis. A CSM searching for Blackstone gets renewal timeline, health score, open support tickets, champion contacts, and expansion signals — not a generic company overview.
...
1. With CS preset active and Glean connected, refreshing Blackstone produces queries like `"Blackstone renewal"`, `"Blackstone health score"`, `"Blackstone champion"` — not just `"Blackstone account"`
