...
These return broad, unfocused results. When tested against a real Glean instance (Globex Holdings account), the generic `"{name} account"` query returned a mix of REDACTED data, commercials, key roles, support signals, and account activity — but Glean itself suggested more targeted queries would yield better results.
...
|---|---|---|
| REDACTED opportunity data | Vitals (ARR, renewal date) | Could auto-populate vitals from Glean |
...
| REDACTED contacts | Stakeholder map | Auto-discover stakeholders |
...
- Structured data extraction from Glean results (mapping REDACTED fields to vitals) — future issue
