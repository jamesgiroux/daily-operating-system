...
When a meeting is tagged to a parent account (e.g., "Cox Enterprises"), the meeting may contain signals relevant to specific child accounts (e.g., "Cox B2B" and "Cox Retail" are mentioned separately in the transcript). Currently, all signals from that meeting land on the parent entity and stay there — the transcript processor doesn't perform content-level entity resolution to route signals to the appropriate children.
