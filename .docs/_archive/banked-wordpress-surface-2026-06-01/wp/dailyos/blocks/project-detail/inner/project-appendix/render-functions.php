<?php
/**
 * Project Appendix (project-appendix) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: envelope_provenance (envelope sections projected by this chapter).
...
if ( ! function_exists( 'dailyos_project_appendix_render' ) ) {
	/**
	 * Render the project-appendix inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_project_appendix_render( array $attributes, string $content = '', $block = null ): string {
		...
		$projected_sections = [ 'envelope_provenance' ];
		$any_present        = false;
		$first_empty_reason = '';
		...
		if ( ! $any_present ) {
			$reason = '' !== $first_empty_reason ? $first_empty_reason : 'no_appendix_data';
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No appendix data yet', 'dailyos' ),
					'wp-block-dailyos-project-appendix'
				);
			}
			return '<section class="wp-block-dailyos-project-appendix"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No appendix data yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-project-appendix',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'project-appendix',
					'data-dailyos-trust-band-src' => 'aggregate',
				]
			)
			: 'class="wp-block-dailyos-project-appendix" data-dailyos-trust-band-src="aggregate"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-project-appendix-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
