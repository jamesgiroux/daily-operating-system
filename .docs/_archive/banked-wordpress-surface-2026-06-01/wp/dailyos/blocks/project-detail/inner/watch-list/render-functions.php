<?php
/**
 * Watch List (watch-list) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: open_loops (envelope sections projected by this chapter).
 * Trust band source: per_loop.
 *
...
if ( ! function_exists( 'dailyos_watch_list_render' ) ) {
	/**
	 * Render the watch-list inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_watch_list_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'Nothing on the watch list', 'dailyos' ),
					'wp-block-dailyos-watch-list'
				);
			}
			return '<section class="wp-block-dailyos-watch-list"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'Nothing on the watch list', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-watch-list',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'watch-list',
					'data-dailyos-trust-band-src' => 'per_loop',
				]
			)
			: 'class="wp-block-dailyos-watch-list" data-dailyos-trust-band-src="per_loop"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-watch-list-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
