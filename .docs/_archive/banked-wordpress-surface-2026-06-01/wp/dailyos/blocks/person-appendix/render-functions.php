<?php
/**
 * Person Appendix inner-block server-side render (W2 L1 — DOS-484).
 *
 * Projects from envelope section(s): envelope-level EnvelopeProvenance + all section caveats.
...
if ( ! function_exists( 'dailyos_person_appendix_render' ) ) {
	/**
	 * Render the Person Appendix inner block.
	 *
	 ...
	 */
	function dailyos_person_appendix_render( array $attributes, array $ctx = [] ): string {
		...
		$wrapper_attrs = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'        => 'wp-block-dailyos-person-appendix',
		...
		$out .= '<div class="dailyos-person-appendix__body" data-dailyos-projection="person-appendix" data-trust-band-source="aggregate"></div>';
		$out .= '</section>';
