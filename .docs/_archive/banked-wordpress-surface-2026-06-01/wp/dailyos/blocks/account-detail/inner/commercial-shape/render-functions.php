<?php
/**
 * Commercial Shape (commercial-shape) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (commercial shape narrative).
 *
...
if ( ! function_exists( 'dailyos_commercial_shape_render' ) ) {
	/**
	 * Render the commercial-shape inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_commercial_shape_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_commercial_shape_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the commercial-shape projection.
	 ...
	 */
	function dailyos_commercial_shape_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			$envelope,
			[ 'facts' ],
			[
				'field_paths'         => [ 'contractContext', 'agreementOutlook' ],
				'field_path_prefixes' => [ 'expansionSignals[' ],
			]
		);
	}
}

if ( ! function_exists( 'dailyos_commercial_shape_render_row' ) ) {
	/**
	 * Render a single claim row inside the commercial-shape projection.
	...
	function dailyos_commercial_shape_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<div class="ReferenceGrid_row" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			. '<span class="ReferenceGrid_label">' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</span>'
