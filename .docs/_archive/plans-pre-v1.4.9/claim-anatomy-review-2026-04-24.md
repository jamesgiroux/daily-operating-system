...
**Effective behavior today.** Every claim freshly synthesized from a 6-month-old REDACTED note looks fresh. Trust Compiler `freshness_weight` (DOS-10) will be calibrated against ingestion time, not evidence time.
...
- **Sensitivity inheritance from sources.** A claim sourced from an Internal REDACTED note should default `Internal`. v1.4.0 ships defaults but no automatic source-class inheritance. v1.4.1 adds inheritance via `ClaimTypeMetadata.default_sensitivity`.
