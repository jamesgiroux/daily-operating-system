---
area: "Accounts"
account: "{{ACCOUNT_NAME}}"
doc_type: "report"
date: "{{DATE}}"
status: "active"
privacy: "internal"
tags: ["dashboard", "account-health"]
report_type: "account-dashboard"
---

# {{ACCOUNT_NAME}} Dashboard
*Last Updated: {{DATE}}*
*Account Owner: {{ACCOUNT_OWNER}}*
*Status: {{STATUS_EMOJI}} {{STATUS}}*

---

## Quick View
| Metric | Value | Notes |
|--------|-------|-------|
| **ARR/Contract Value** | {{ARR}} | |
| **Health Score** | {{HEALTH_SCORE}} | |
| **Account Status** | {{STATUS}} | |
| **Meeting Cadence** | {{MEETING_CADENCE}} | Last: {{LAST_ENGAGEMENT}} |
| **Renewal Date** | {{RENEWAL_DATE}} | {{DAYS_TO_RENEWAL}} |
| **Support Package** | {{SUPPORT_PACKAGE}} | |
| **Primary Contact** | {{PRIMARY_CONTACT}} | |

---

## Critical Information

### ⚠️ Current Risks
<!-- List any current risks or concerns with this account -->
-

### ✅ Recent Wins
<!-- Document recent successes, positive outcomes, or value delivered -->
-

### 🎯 Next Actions
<!-- What needs to happen next? Include owners and due dates -->
- [ ]

---

## Account Intelligence

### Business Context
<!-- Industry, company overview, business model, strategic priorities -->
**Industry**:
**Company Overview**:
**Business Model**:
**Strategic Priority**:
**Relationship Type**:

### Current State

**Working Well**:
<!-- What's going smoothly? Positive engagement signals? -->
-

**Challenges**:
<!-- What obstacles exist? What's causing friction? -->
-

**Unknown / Discovery Needed**:
<!-- What questions need answers? What gaps exist in your knowledge? -->
-

---

## Stakeholder Map

| Name | Role | Influence | Engagement Level | Last Contact | Notes |
|------|------|-----------|------------------|--------------|-------|
| {{PRIMARY_CONTACT}} | | High | Active | {{LAST_ENGAGEMENT}} | Primary contact |
| | | | | | |

### Missing Coverage (Discovery Needed)
<!-- Who should you know but don't? Executive sponsors? Decision makers? -->
-

---

## Value Tracking

### Success Metrics
<!-- What does success look like for this customer? How do you measure value? -->

| Metric | Current | Target | VIP Contribution |
|--------|---------|--------|------------------|
| | | | |

### Value Delivered
<!-- Document specific value you've delivered with dates and impact -->

| Date | Value Statement | Source | Impact |
|------|-----------------|--------|--------|
| | | | |

### Value Gaps
<!-- Where could you be delivering more value? What opportunities exist? -->
-

---

## Technical Landscape

### Current Architecture
<!-- What systems, platforms, integrations are in place? -->
**Platform**:
**Key Integrations**:
**Technical Health**:

### Technical Debt & Issues
<!-- Known issues, planned upgrades, technical concerns -->

| Priority | Issue | Status | Owner | Due Date |
|----------|-------|--------|-------|----------|
| | | | | |

---

## Engagement Tracking

### Meeting Cadence
**Expected Frequency**: {{MEETING_CADENCE}}
**Actual Frequency (90 days)**:
<!-- List recent meetings -->

### Engagement Signals

| Signal Type | Assessment | Evidence |
|-------------|------------|----------|
| Support Ticket Volume | | |
| Feature Adoption | | |
| Proactive Engagement | | |
| Executive Engagement | | |

---

## Commercial Information

### Contract Details
- **Current ARR:** {{ARR}}
- **Last Renewal:** {{LAST_RENEWAL}}
- **Next Renewal:** {{RENEWAL_DATE}}
- **Contract Type:** {{SUPPORT_PACKAGE}}

### Growth Opportunities
| Opportunity | Value | Probability | Timeline | Next Step |
|-------------|-------|-------------|----------|-----------|
| | | | | |

### Renewal Strategy
- **Days Until Renewal:** {{DAYS_TO_RENEWAL}}
- **Renewal Confidence:**
- **Key Risks:**
- **Key Opportunities:**

---

## Success Plan

**Active Success Plan:** Link to success plan document
`Accounts/{{ACCOUNT_NAME}}/01-Customer-Information/{{ACCOUNT_SLUG}}-success-plan.md`

- **Current Objectives:**
- **Last Updated:**
- **Next Review:**

---

## Historical Context

### Key Events Timeline
<!-- Major milestones, incidents, decisions -->

| Date | Event | Impact | Source |
|------|-------|--------|--------|
| | | | |

### Escalations/Issues History

| Date | Issue | Resolution | Impact |
|------|-------|------------|--------|
| | | | |

---

## Resources & Links

### Internal Resources
- **Account Folder**: `Accounts/{{ACCOUNT_NAME}}/`
- **Account Index**: [00-Index.md](../00-Index.md)

### Recent Meetings (Last 90 Days)
<!-- Links to meeting summaries and transcripts -->
-

---

## Account Team Notes

### Open Questions for Discovery
<!-- What do you need to learn about this account? -->
1.

### Internal Alignment Notes
<!-- Coordination with sales, support, other internal teams -->
-

### Personal Relationship Notes
<!-- Communication preferences, personal details, rapport building notes -->
-

---

## Review Checklists

### Monthly Review Checklist
- [ ] Review any touchpoints this month
- [ ] Update dashboard with new insights
- [ ] Check engagement health signals
- [ ] Update CRM/system of record with changes

### Quarterly Review Checklist
- [ ] Conduct strategic touchpoint
- [ ] Review and update success plan objectives
- [ ] Assess account health status
- [ ] Update renewal strategy if needed
- [ ] Document value delivered this quarter
- [ ] Plan next quarter's focus

---

*Dashboard created: {{DATE}}*
