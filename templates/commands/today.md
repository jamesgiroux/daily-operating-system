...
multi_bu_domains = ['REDACTED.com', 'hilton.com', 'crestview-media.com']
```

**Multi-BU Accounts** (require title-matching or user prompt):
- `REDACTED.com` → 8 BUs (Digital Experience, AppExchange, Engineering, etc.)
- `hilton.com` → 5 BUs (Domestic, Careers, Corporate, B2B Engagement, Newsroom)
- `crestview-media.com` → 4 BUs (BrandsCo, Corporate Services B2B, Diversification, Enterprises)
...
- "Agentforce" → Projects/Agentforce/ (partners: @REDACTED.com, @rtcamp.com)
...
ELSE (external attendees present):
    ...
    ELSE IF multiple accounts matched (e.g., @REDACTED.com with 8 BUs):
        type = "customer"
...
|---------|---------------|-----------------|----------|
| Agentforce | "Agentforce", "VIP <> REDACTED" | @REDACTED.com, @rtcamp.com | Projects/Agentforce/ |
...
1. **Load Account Dashboard** (PRIMARY SOURCE):
   ...
   # OR for multi-BU:
   ls Accounts/REDACTED/ | grep -i "[BU-Name]"
   ```
   ```
...
- [ ] **Review Gustavo's Node.js/log analysis support ticket** - Owner: James - Due: Dec 18
  ...
  - **Requested by**: Renan (on behalf of Gustavo at Nexus Analytics)
  - **Source**: `Accounts/Nexus Analytics/02-Meetings/2025-12-15-summary-dataco-vip-sync.md`
...
2. **Find Shared Accounts** (if applicable):
   ...
   # Returns: [{'account': 'REDACTED DMT', 'ring': 'Summit', 'your_actions': [...], 'their_actions': [...]}]
   ```
...
├── dataco-2026-01-12.md
├── globex-2026-01-15.md
└── REDACTED-digital-experience-2026-01-16.md
```
...
| 12:00 PM | **Nexus Analytics Monthly** | **Customer** | See 04-1200-customer-dataco-prep.md |
...
|------------|---------|----------|
| Driver/Passenger | Amy/James: Globex Holdings (11am) | Ask "what do you need from me?" first |
| Surface Unknowns | Nexus Analytics docs prep | Lead with "what we don't know" |
...
├── 04-1200-customer-dataco-prep.md    # Customer meeting (FULL PREP)
...
│   └── globex-jan-12.md            # Agenda to review and send
...
1. Catch-up: Nexus Analytics notes (12:30 PM - 12:45 PM)
   Purpose: Document meeting outcomes, update action items

2. Agenda: Globex Holdings (Jan 9, 9:00 AM)
   Purpose: Review and send meeting agenda
...
```json
{
  ...
  "mappings": [
    {
      "domain": "REDACTED.com",
      "attendee_pattern": "sarah.chen@REDACTED.com",
  ...
  "default_bus": {
    "REDACTED.com": "Digital-Experience",
    "hilton.com": "Domestic",
    "crestview-media.com": "Corporate-Services-B2B",
    "quickware.com": "CreditCo"
  }
}
```
...
if domain in ['REDACTED.com', 'hilton.com', 'crestview-media.com', 'quickware.com']:
