...
Add `parent_id` self-referential FK on accounts table. Enterprise accounts (Crestview Media, Harbor Group, Salesforce, Infocalc) contain business units as child accounts, each with independent contracts, renewal dates, health, and lifecycle. Nested workspace directories (`Accounts/Crestview Media/BrandsCo/`). UI: expandable parent rows on AccountsPage, BU detail with breadcrumb navigation. Intelligence aggregation rolls up child signals to parent. Migration: non-breaking (`parent_id` nullable, existing flat accounts unaffected).
