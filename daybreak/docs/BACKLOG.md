...
Add `parent_id` self-referential FK on accounts table. Enterprise accounts (Crestview Media, Harbor Group, REDACTED, Infocalc) contain business units as child accounts, each with independent contracts, renewal dates, health, and lifecycle. Nested workspace directories (`Accounts/Crestview Media/REDACTED/`). UI: expandable parent rows on AccountsPage, BU detail with breadcrumb navigation. Intelligence aggregation rolls up child signals to parent. Migration: non-breaking (`parent_id` nullable, existing flat accounts unaffected).
...
Build MCP client infrastructure in Rust for consuming external data sources per ADR-0046 and ADR-0027. Start with one integration per category: one transcript source (Gong or Granola), one CRM (REDACTED), one task tool (Linear). Evolves I28 (MCP client side).
...
- **Workspace structure:** `Accounts/REDACTED/Engineering/dashboard.json` — nested directory. Parent has its own `dashboard.json` at `Accounts/REDACTED/dashboard.json`. `sync_accounts_from_workspace()` detects two-level nesting: if a subdirectory contains `dashboard.json` OR doesn't match the numbered `\d+-` pattern, it's a BU child.
