...
///
/// Child accounts have `tracker_path` like `Accounts/Crestview Media/REDACTED` which
...
/// accounts (`Accounts/Crestview Media/REDACTED/dashboard.json`, or deeper).
///
...
fn scan_child_accounts_inner(
    ...
    for sub_entry in sub_entries {
        ...
        if child_json_path.exists() {
            ...
            let (child_id, _child_name, _child_parent_id, child_tracker) =
            ...
            if db.get_account(&child_id).ok().flatten().is_none() {
                // Unrecognized child folder — skip instead of auto-bootstrapping
                log::info!(
                    "Skipping unrecognized child account folder '{}'",
                    child_tracker,
                );
            }
        }
...
END_ENRICHMENT";
    ...
    fn test_is_bu_directory() {
        assert!(is_bu_directory("REDACTED"));
    ...
    fn test_child_id_scheme() {
        ...
        assert_eq!(
            format!("{}--{}", slugify("Crestview Media"), slugify("REDACTED")),
            "REDACTED--REDACTED"
        );
    ...
    fn test_read_account_json_nested_child() {
        let dir = tempfile::tempdir().expect("tempdir");
        let workspace = dir.path();
        let child_dir = workspace.join("Accounts/Crestview Media/REDACTED");
        ...
        assert_eq!(result.account.id, "REDACTED--REDACTED");
        assert_eq!(result.account.name, "REDACTED");
        assert_eq!(result.account.parent_id, Some("REDACTED".to_string()));
        assert_eq!(
            result.account.tracker_path,
            Some("Accounts/Crestview Media/REDACTED".to_string())
        );
    ...
    fn test_write_with_tracker_path_uses_correct_dir() {
        ...
        let account = DbAccount {
            id: "REDACTED--REDACTED".to_string(),
            ...
            contract_end: None,
            nps: None,
            tracker_path: Some("Accounts/Crestview Media/REDACTED".to_string()),
            parent_id: Some("REDACTED".to_string()),
            account_type: crate::db::AccountType::Customer,
        ...
        // Should write to nested path, not Accounts/REDACTED/
        let nested = workspace.join("Accounts/Crestview Media/REDACTED/dashboard.json");
        assert!(nested.exists(), "Should write to nested tracker_path");

        let flat = workspace.join("Accounts/REDACTED/dashboard.json");
        assert!(!flat.exists(), "Should NOT write to flat path");
    }

    #[test]
    fn test_sync_skips_bu_subdirectories_without_db_record() {
        let dir = tempfile::tempdir().expect("tempdir");
        let workspace = dir.path();
        let db = test_db();

        // Create parent with dashboard.json (will sync from JSON)
        ...
        // Create BU child dir (no dashboard.json, no DB record — should be SKIPPED)
        ...
        let synced = sync_accounts_from_workspace(workspace, &db).unwrap();
        // Only the parent syncs (from dashboard.json)
        assert_eq!(synced, 1, "Only parent with dashboard.json should sync");
    ...
    fn test_get_child_and_top_level_accounts() {
        ...
        let child2 = DbAccount {
            id: "REDACTED--enterprise".to_string(),
        ...
        assert_eq!(top.len(), 1);
        assert_eq!(top[0].id, "REDACTED");

        // children query
        let children = db.get_child_accounts("REDACTED").unwrap();
        assert_eq!(children.len(), 2);
        assert!(children.iter().any(|c| c.id == "REDACTED--REDACTED"));
        assert!(children.iter().any(|c| c.id == "REDACTED--enterprise"));
    }
