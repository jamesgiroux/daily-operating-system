...
impl WorkspaceGraphDiagnosticKey {
    ...
    fn from_database_key(key: &crate::db::EncryptionKey) -> Self {
        Self::from_install_secret(key.as_hex().as_bytes())
    }

    #[cfg(test)]
    fn for_tests(label: &str) -> Self {
...
pub fn local_install_diagnostic_key() -> Result<WorkspaceGraphDiagnosticKey, String> {
    let db_path = crate::db::ActionDb::db_path_public()
        .map_err(|error| format!("database path unavailable: {error}"))?;
    let provider = crate::db::LocalKeychain::new();
    let user = crate::db::UserIdentity::local(db_path);
    let key = crate::db::DbKeyProvider::get_or_create_key(&provider, &user)
        .map_err(|error| format!("diagnostic key unavailable: {error}"))?;
    Ok(WorkspaceGraphDiagnosticKey::from_database_key(&key))
}
    ...
    fn insert_claim_with_subject(
        ...
        conn.execute(
            ...
            params![
                claim_id,
                subject_ref,
                format!("{WORKSPACE_SOURCE_PREFIX}{file_id}"),
                trust_score,
                sensitivity
            ],
        )
    ...
    fn db_shaped_pascal_case_subject_matches_semantic_evidence_source() {
        ...
        conn.execute(
            "UPDATE intelligence_claims
             SET source_ref = 'manual:source'
             WHERE id = 'claim-1'",
            [],
        )
        .expect("detach direct workspace source");
        conn.execute(
