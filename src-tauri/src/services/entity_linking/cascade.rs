...
pub fn run_cascade(
    ...
    if let Some(ref primary) = primary_entity {
        if primary.entity_type == "account" && link_ctx.owner.owner_type == OwnerType::Meeting {
            c6_backfill_account_domains(ctx, link_ctx, &primary.entity_id, db)?;
        }
    }
...
fn c6_backfill_account_domains(
    ctx: &ServiceContext<'_>,
    link_ctx: &LinkingContext,
    account_id: &str,
    db: &ActionDb,
    ...
    for p in &link_ctx.participants {
...
fn c3_promote_trusted_stakeholders(
    ctx: &ServiceContext<'_>,
    link_ctx: &LinkingContext,
    account_id: &str,
    db: &ActionDb,
...
mod tests {
    ...
    fn c6_merges_external_attendee_domains_for_meeting_primary() {
        ...
        run_cascade(&service_ctx, &ctx, &primary, &[], &db).expect("cascade");
        ...
        run_cascade(&service_ctx, &ctx, &primary, &[], &db).expect("cascade rerun");
    ...
    fn c6_does_not_run_for_person_primary() {
        ...
        run_cascade(&service_ctx, &ctx, &person_primary, &[], &db).expect("cascade");
