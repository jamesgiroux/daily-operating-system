...
pub async fn create_project(
    ...
    state.db_write(move |db| {
        ...
        if let Some(ref config) = config {
            ...
            let _ = crate::util::bootstrap_entity_directory(&project_dir, &validated_name_clone, "project");
...
pub async fn update_project_field(
    ...
    state.db_write(move |db| {
        ...
        let _ = crate::self_healing::scheduler::evaluate_on_signal(db, &project_id, "project", &intel_queue);

        // Self-healing: record user correction for enrichable fields (I409)
        if matches!(field.as_str(), "status" | "milestone" | "owner" | "target_date") {
