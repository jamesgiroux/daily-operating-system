...
/// Wire-shape DTO for a single tool entry in `tool_descriptions.yaml`.
...
/// Embedded YAML catalog compiled into the binary. Production uses
/// [`YamlTaxonomyCatalog::load_embedded`]; dev can override via
/// `DAILYOS_MCP_TAXONOMY_PATH` env var pointed at a local YAML file.
const EMBEDDED_YAML: &str = include_str!("../../../resources/mcp_v2/tool_descriptions.yaml");
...
impl YamlTaxonomyCatalog {
    ...
    pub fn load_from_str(yaml: &str) -> Result<Self, TaxonomyError> {
        let raw: Vec<YamlToolEntry> = serde_yaml::from_str(yaml).map_err(|e| {
            TaxonomyError::ParseFailed {
                error: e.to_string(),
            }
        })?;
...
"#;
        match YamlTaxonomyCatalog::load_from_str(yaml) {
...
- name: dailyos.read.foo
  ...
  when_to_call: y
  when_NOT_to_call: z
  scopesRequired: [some.random.scope]
  parameters: []
  returns: { schema: {}, description: x }
  examples: []
  selectionFixtures:
...
- name: dailyos.read.foo
  ...
  examples: []
  selectionFixtures:
    positive: [{ prompt: a, expectedTool: dailyos.read.foo }]
    negativeBroadCorpus: [{ prompt: c, expectedToolClass: external }, { prompt: d, expectedToolClass: external }]
    negativeAdjacentTool: [{ prompt: e, expectedTool: dailyos.read.bar }]
"#;
        match YamlTaxonomyCatalog::load_from_str(yaml) {
