...
fn seed_intelligence_data(db: &ActionDb) -> Result<(), String> {
    ...
    let acme_intel = IntelligenceJson {
        ...
        value_delivered: vec![
            ValueItem { date: Some(days_ago_rfc(90)), statement: "Phase 1 deployment drove $200K ARR expansion".into(), source: Some("contract".into()), impact: Some("High".into()), item_source: Some(ItemSource { source: "glean_crm".into(), confidence: 0.9, sourced_at: days_ago_rfc(90), reference: Some("REDACTED".into()) }), discrepancy: None, ..Default::default() },
