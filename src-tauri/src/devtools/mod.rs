...
/// Seed Glean-enriched intelligence data: Gong summaries, REDACTED context, support health, product adoption.
fn seed_glean_enriched_data(db: &ActionDb) -> Result<(), String> {
    ...
    // Update mock-globex-industries with REDACTED context and Gong data
    ...
    log::info!("seed_glean_enriched_data: Gong + REDACTED + Zendesk + adoption data patched");
    Ok(())
}
...
fn seed_intelligence_data(db: &ActionDb) -> Result<(), String> {
    ...
    let acme_intel = IntelligenceJson {
        ...
        value_delivered: vec![
            ...
            ValueItem { date: Some(days_ago_rfc(60)), statement: "Performance benchmarks exceeded targets by 15%".into(), source: Some("analytics".into()), impact: Some("cost".into()), item_source: Some(ItemSource { source: "glean_crm".into(), confidence: 0.85, sourced_at: days_ago_rfc(60), reference: Some("REDACTED".into()) }), discrepancy: None },
        ],
