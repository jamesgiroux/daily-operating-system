...
pub fn gather_email_context(
    ...
    // Randall's REDACTED-scoped signals were the most recent rows in the table.
    //
...
mod tests {
    ...
    /// REDACTED-scoped email signals in the briefing digest.
    #[test]
    fn pre_meeting_context_signals_are_entity_scoped() {
        ...
        for (eid, from, text) in [
            ("REDACTED", "randall.benson@example.com", "Randall: REDACTED thread"),
            ("REDACTED", "randall.benson@example.com", "Randall: another REDACTED thread"),
        ...
        assert_eq!(
            arr.len(),
            1,
            "josiah meeting must not pull REDACTED signals; got {arr:?}"
        );
        assert_eq!(
        ...
        // Gather for REDACTED — must see both REDACTED rows.
        let result = gather_email_context(&db, &[], "REDACTED", 8);
        let arr = result.as_array().expect("array");
        assert_eq!(arr.len(), 2, "REDACTED should have 2 signals, got {arr:?}");
    }
}
