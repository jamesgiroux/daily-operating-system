...
impl SurfaceEndpointState {
    ...
    async fn start_listener(
        ...
        if let Some(state) = app_state.as_ref() {
            if let Err(err) = rehydrate_sessions_from_keychain(state, &self.signed_transport).await
            {
        ...
        for _attempt in 1..=max_attempts {
            match TcpListener::bind((Ipv4Addr::LOCALHOST, 0)).await {
                Ok(listener) => {
                    ...
                    if let Err(err) = write_runtime_sentinel(bound_port, env!("CARGO_PKG_VERSION"))
                    {
        ...
        match crate::services::surface_session_keychain::load_session_master_key(
            ...
            SessionKeyLookup::Found(master_key) => {
                ...
                if let Err(err) = signed_transport.register_session(session) {
                    log::warn!("surface session rehydrate failed to register session: {err:?}");
                    continue;
                }
                rehydrated += 1;
            }
...
/// Re-grant scopes (union with the registry-derived default grant) to the
/// calling SurfaceClient without invalidating its HMAC session key (scope-
/// refresh mechanism #2).
