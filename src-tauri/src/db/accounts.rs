...
impl ActionDb {
    ...
    /// I651: Upsert product classification from Glean/REDACTED.
...
mod dos258_reclassify_tests {
    ...
    fn reclassify_weak_primary_customer_flips_to_internal() {
        ...
        db.conn_ref()
            .execute(
                "INSERT INTO meetings (id, title, meeting_type, start_time, attendees, created_at, calendar_event_id) \
                 VALUES ('m-weak', 'WordPress VIP Plan', 'customer', '2026-04-20', '[\"me@company.com\",\"jane@jane.app\"]', '2026-04-20', 'ev1')",
                [],
            )
            .expect("seed meeting");
        db.conn_ref()
        ...
        // Now seed a stakeholder link for jane@jane.app on acc-wp and re-flip
        ...
        db.conn_ref()
            .execute(
                "INSERT INTO people (id, name, email, relationship, updated_at) \
                 VALUES ('p-jane', 'Jane', 'jane@jane.app', 'external', '2026-04-20')",
                [],
            )
            .expect("seed person");
        db.conn_ref()
