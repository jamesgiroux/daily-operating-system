...
pub fn classify_meeting_multi(
    ...
    // This allows title-based entity matches (e.g., "Meridian Asset 1:1") to inform
    ...
    if !has_external {
        ...
        // attendees are internal (e.g., "Meridian Asset 1:1" with internal attendees).
...
mod tests {
    ...
    fn test_classify_internal_1on1_with_account_in_title() {
        let hints = vec![account_hint_with_domain("janus-id", "Meridian Asset", &["janushenderson.com"])];
        ...
        // Should be promoted to Entity tier because "Meridian Asset" is a known account
        assert_eq!(result.meeting_type, "one_on_one");
        assert_eq!(result.intelligence_tier, IntelligenceTier::Entity);
        assert!(!result.resolved_entities.is_empty());
        assert_eq!(result.resolved_entities[0].entity_type, "account");
        assert_eq!(result.resolved_entities[0].name, "Meridian Asset");
    }
