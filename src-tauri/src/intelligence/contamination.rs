...
//!
//! TODO(DOS-282): Regression fixtures for vip2-REDACTED.com / REDACTED cross-bleed
...
/// Matches `vip-foo.com`, `vip2-REDACTED.com`, and `vip5.something.com` — the host
...
mod tests {
    ...
    fn detects_foreign_domain_in_text() {
        ...
        insert_account(&db, "other", "REDACTED", &["REDACTED.com", "REDACTED.com"]);

        let text =
            "The latest review mentions vip2-REDACTED.com performance and REDACTED.com customer success.";
        ...
        let domains: Vec<&str> = hits
            ...
            .collect();
        assert!(domains.contains(&"REDACTED.com"));
    }
    ...
    fn allows_target_domain_in_text() {
        ...
        insert_account(&db, "other", "REDACTED", &["REDACTED.com"]);
    ...
    fn detects_wpvip_host_pattern_not_in_target_domains() {
        let db = test_db();
        insert_account(&db, "target", "Jane", &["jane.com"]);
        // Note: REDACTED.com is NOT registered as an account domain — we still want to
        // flag vip2-REDACTED.com as a foreign WP-VIP host.
        let text = "Performance at vip2-REDACTED.com is concerning.";
        let hits =
            detect_cross_entity_contamination(text, "target", &["jane.com".into()], &[], &db);
        assert!(
            hits.iter()
                .any(|h| h.kind == ContaminationKind::InfrastructureId
                    && h.foreign_token == "vip2-REDACTED.com"),
            "expected InfrastructureId hit for vip2-REDACTED.com, got {:?}",
            hits
        );
    }
    ...
    fn case_insensitive_matching() {
        ...
        let text = "Reference to REDACTED.COM in the notes.";
        let hits = detect_cross_entity_contamination(text, "target", &["jane.com".into()], &[], &db);
        assert!(hits
            .iter()
            .any(|h| h.kind == ContaminationKind::Domain && h.foreign_token == "REDACTED.com"));
    }
    ...
    fn collect_narrative_text_pulls_all_prose_fields() {
        ...
        let intel = IntelligenceJson {
            ...
            risks: vec![IntelRisk {
                text: "Churn risk on vip2-REDACTED.com".into(),
                source: None,
                urgency: "watch".into(),
                item_source: None,
                discrepancy: None,
        ...
        assert!(narrative.contains("Churn risk on vip2-REDACTED.com"));
    ...
    fn rejects_write_when_output_contains_foreign_domain() {
        ...
        let db = test_db();
        insert_account(&db, "target", "Jane", &["jane.com"]);
        insert_account(&db, "REDACTED", "REDACTED", &["vip2-REDACTED.com", "REDACTED.com"]);

        let intel = IntelligenceJson {
            executive_assessment: Some(
                "WordPress VIP performance at vip2-REDACTED.com remains stable.".into(),
            ),
            ..Default::default()
        };
        let narrative = collect_narrative_text(&intel);
        ...
        assert!(hits
            .iter()
            .any(|h| h.foreign_token == "vip2-REDACTED.com" || h.foreign_token == "REDACTED"
    ...
    fn allows_write_when_output_is_clean() {
        ...
        let db = test_db();
        insert_account(&db, "target", "Jane", &["jane.com"]);
        insert_account(&db, "REDACTED", "REDACTED", &["vip2-REDACTED.com"]);
    ...
    fn reject_hit_carries_source_account_id_when_attributable() {
        let db = test_db();
        insert_account(&db, "target", "Jane", &["jane.com"]);
        insert_account(&db, "REDACTED", "REDACTED", &["REDACTED.com"]);

        let text = "Reference to REDACTED.com in the notes.";
        let hits = detect_cross_entity_contamination(text, "target", &["jane.com".into()], &[], &db);
        let REDACTED_hit = hits.iter().find(|h| h.foreign_token == "REDACTED.com").unwrap();
        assert_eq!(REDACTED_hit.source_account_id.as_deref(), Some("REDACTED"));
    }
    ...
    fn whole_word_bounded_not_substring() {
        ...
        // "REDACTED.com" as a pure substring inside another word shouldn't match.
        let text = "See prefix-REDACTED.com-suffix.foo.";
        let hits = detect_cross_entity_contamination(text, "target", &["jane.com".into()], &[], &db);
        ...
        assert!(
            hits.iter().any(|h| h.foreign_token == "REDACTED.com"),
            "expected REDACTED.com to match at non-alphanumeric boundary"
        );
    }
}
