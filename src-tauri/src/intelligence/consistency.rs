...
mod tests {
    ...
    fn no_false_positive_for_parent_child_reference() {
        // "REDACTED" is in related_entity_names, NOT in all_other_entity_names
        let facts = FactContext {
            entity_id: "REDACTED-bu".to_string(),
            entity_type: "account".to_string(),
            entity_name: "REDACTED BU".to_string(),
            related_entity_names: vec!["REDACTED".to_string()],
            all_other_entity_names: vec!["Clevertap".to_string()],
            stakeholders: vec![],
            recent_signal_count_14d: 0,
        };
