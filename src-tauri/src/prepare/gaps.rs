...
/// Parse an ISO datetime string to a NaiveDateTime (strip timezone info).
fn parse_event_dt(time_str: &str) -> Option<NaiveDateTime> {
    if time_str.is_empty() {
        return None;
    }
    if time_str.contains('T') {
        // ISO datetime: "2026-02-08T09:00:00-05:00" or "2026-02-08T14:00:00Z"
...
pub fn compute_gaps(events: &[Value], day_date: NaiveDate) -> Vec<Value> {
...
pub fn compute_all_gaps(
    ...
    for (i, day_name) in DAY_NAMES.iter().enumerate() {
        ...
        result.insert(
            day_name.to_string(),
            Value::Array(compute_gaps(&day_events, day_date)),
        );
    }
    Value::Object(result)
}
...
pub fn suggest_focus_blocks(gaps_by_day: &Value) -> Vec<Value> {
    ...
    for day_name in DAY_NAMES {
        ...
        for gap in day_gaps {
            ...
            let start_dt = match parse_event_dt(start_str) {
                Some(dt) => dt,
                None => continue,
            };
...
mod tests {
    ...
    fn test_parse_event_dt_iso() {
        let dt = parse_event_dt("2026-02-08T09:00:00-05:00");
        assert!(dt.is_some());
    ...
    fn test_parse_event_dt_utc() {
        let dt = parse_event_dt("2026-02-08T14:00:00Z");
        assert!(dt.is_some());
    ...
    fn test_parse_event_dt_date_only() {
        let dt = parse_event_dt("2026-02-08");
        assert!(dt.is_some());
    ...
    fn test_compute_gaps_one_meeting() {
        ...
        let gaps = compute_gaps(&events, day);
    ...
    fn test_compute_all_gaps() {
        ...
        let result = compute_all_gaps(&events_by_day, monday);
