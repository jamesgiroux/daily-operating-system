...
fn build_fuzzy_tokens(text: &str) -> Vec<String> {
    ...
    // Adjacent pairs (e.g. "sales force" for matching "REDACTED")
...
mod tests {
    ...
    fn test_fuzzy_matching_jaro_winkler() {
        // "REDACTED" vs "salesforc" (typo) should match
        assert!(strsim::jaro_winkler("REDACTED", "salesforc") >= 0.85);
        // Completely different strings should not match
        assert!(strsim::jaro_winkler("REDACTED", "microsoft") < 0.85);
    ...
    fn test_fuzzy_matches_tokens() {
        ...
        // "REDACTED" should fuzzy-match "salesforc" token
        assert!(fuzzy_matches_tokens("REDACTED", &tokens));
