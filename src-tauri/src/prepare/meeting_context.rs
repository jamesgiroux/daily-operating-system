...
fn resolve_account_identifier(
    ...
    // Prefer path resolution from slug-like IDs (e.g., "REDACTED--digital-marketing-technology")
...
///
/// Example: `REDACTED--digital-marketing-technology` -> `REDACTED/Digital-Marketing-Technology`.
...
pub(crate) struct AccountMatch {
    /// Display name (e.g., "REDACTED" for a child, "Crestview Media" for a parent).
    pub(crate) name: String,
    /// Relative path from Accounts/ dir (e.g., "Crestview Media/REDACTED" for a child, "Crestview Media" for flat).
...
mod tests {
    ...
    fn test_guess_account_name_child_bu() {
        ...
        std::fs::create_dir_all(dir.path().join("Crestview Media/REDACTED")).unwrap();

        // Match by domain (most common for BU meetings)
        let meeting = json!({
            "title": "Weekly Sync",
            "external_domains": ["REDACTED.REDACTED.com"],
        });
        let matched = guess_account_name(&meeting, dir.path()).unwrap();
        assert_eq!(matched.name, "REDACTED");
        assert_eq!(matched.relative_path, "Crestview Media/REDACTED");
    }

    #[test]
    fn test_guess_account_name_child_by_title() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join("Crestview Media/Enterprise")).unwrap();
        ...
        assert_eq!(matched.relative_path, "Crestview Media/Enterprise");
    }

    #[test]
    fn test_guess_account_name_child_by_title_normalized() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join("REDACTED/Digital-Marketing-Technology"))
            .unwrap();
        ...
        assert_eq!(
            ...
            "REDACTED/Digital-Marketing-Technology"
        );
    }
    ...
    fn test_guess_account_name_child_by_domain() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join("REDACTED/Engineering")).unwrap();

        let meeting = json!({
            "title": "Weekly Sync",
            "external_domains": ["engineering.REDACTED.com"],
        });
        let matched = guess_account_name(&meeting, dir.path()).unwrap();
        assert_eq!(matched.name, "Engineering");
        assert_eq!(matched.relative_path, "REDACTED/Engineering");
    }
    ...
    fn test_find_account_dir_by_id_hint_prefers_parent_child_path() {
        ...
        let matched =
            find_account_dir_by_id_hint("REDACTED--digital-marketing-technology", dir.path())
    ...
    fn test_resolve_account_from_db_prefers_meeting_account_id() {
        ...
        std::fs::create_dir_all(
            dir.path()
                .join("Accounts/REDACTED/Digital-Marketing-Technology"),
        )
        .unwrap();
