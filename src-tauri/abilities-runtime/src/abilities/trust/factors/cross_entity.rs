...
use super::super::types::{CrossEntityCoherenceInput, CrossEntityHit, CrossEntityHitKind};
...
pub fn cross_entity_coherence(
    ...
    let value = if hits.is_empty() {
        ...
        (FACTOR_MAX - config.cross_entity_hit_penalty).powi(hits.len() as i32)
    };
