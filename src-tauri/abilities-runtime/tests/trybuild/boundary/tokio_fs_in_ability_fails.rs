...
async fn tokio_fs_boundary(ctx: &AbilityContext<'_>, input: Input) -> AbilityResult<Output> {
    use tokio::fs;

    let _ = (ctx.mode(), input);
    let _ = fs::write("target/ability-runtime-boundary-proof", b"forbidden").await;
    unimplemented!()
}
