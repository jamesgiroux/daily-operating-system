/**
 * Accounts index — client-side hydration.
 *
 * Mounts a React shell at every `[data-dailyos-accounts-index-mount]` node.
 * Consumes the shared `useAbilityCursor` hook (registered globally on
 * `window.wp.dailyosShared` by `blocks/_shared/hooks/use-ability-cursor.js`)
 * to drive cursor pagination against the `list_accounts` ability.
 *
 * Per L0 Packet W2 §5.5 AC-L.1–L.6.
 */
...
( function ( wp ) {
	...
	var MOUNT_SELECTOR = '[data-dailyos-accounts-index-mount]';
	var BLOCK_ROOT_SELECTOR = '.wp-block-dailyos-accounts-index';

	function getConfigForMount( node ) {
		var root = node.closest( BLOCK_ROOT_SELECTOR );
		if ( ! root ) {
			return { pageSize: 25, watermark: '', ability: 'list_accounts' };
		}
		...
		return {
			...
			ability:
				root.getAttribute( 'data-dailyos-ability' ) || 'list_accounts',
		};
	}

	function AccountsIndexShell( props ) {
		...
		var rows = cursor.items.map( function ( item, idx ) {
			...
			return createElement(
				...
				{
					key: key,
					className: 'dailyos-accounts-index__row',
					'data-account-id': item && item.id ? item.id : null,
				},
				label
			);
		} );
		...
		var loadMoreAffordance = cursor.done
			? createElement(
					'span',
					{
						className: 'dailyos-accounts-index__end-mark',
						'aria-hidden': 'true',
					},
					'* * *'
				)
			: createElement(
					...
					{
						type: 'button',
						className: 'dailyos-accounts-index__load-more',
		...
		var restartChip =
			...
			cursor.cursorState.restart_required
				? createElement(
						...
						{
							type: 'button',
							className: 'dailyos-accounts-index__refresh',
		...
		return createElement(
			'div',
			{ className: 'dailyos-accounts-index__shell' },
			advisoryChip,
			createElement(
				'ul',
				{ className: 'dailyos-accounts-index__list' },
				rows
			),
			loadMoreAffordance,
			restartChip
	...
	function mountAll() {
		document.querySelectorAll( MOUNT_SELECTOR ).forEach( function ( node ) {
			if ( node._dailyosAccountsIndexMounted ) return;
			node._dailyosAccountsIndexMounted = true;
			var cfg = getConfigForMount( node );
			node.innerHTML = '';
			var root = createRoot( node );
			root.render(
				createElement( AccountsIndexShell, {
					ability: cfg.ability,
