<?php
/**
 * Open Loops Feed (open-loops-feed) — W2 L1 inner block render-functions.
 *
 * Projection rule: OpenLoops (envelope first page).
 *
	...
	function dailyos_open_loops_feed_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			return dailyos_empty_chip(
				$reason,
				__( 'No open loops yet', 'dailyos' ),
				'wp-block-dailyos-open-loops-feed'
			);
		}

		$wrapper_attrs = dailyos_inner_block_wrapper_attrs( 'wp-block-dailyos-open-loops-feed' );
		$out  = '<div ' . $wrapper_attrs . ' data-dailyos-projection="open-loops-feed">';
		$out .= '<header class="wp-block-dailyos-open-loops-feed__header"><span class="wp-block-dailyos-open-loops-feed__title">' . esc_html__( 'Open Loops Feed', 'dailyos' ) . '</span></header>';
		$out .= '<div class="wp-block-dailyos-open-loops-feed__body" data-dailyos-envelope-sections="' . esc_attr( implode( ',', ['open_loops'] ) ) . '">';
		...
		$projected_claim_refs = dailyos_open_loops_feed_select_claim_refs( $envelope );
		foreach ( $projected_claim_refs as $claim_ref ) {
			$receipt = dailyos_envelope_consume_claim( $claim_ref, $scope_set );
			if ( null === $receipt ) {
				continue;
			}
			$out .= dailyos_open_loops_feed_render_row( $claim_ref, $receipt );
		}
		$out .= '</div>';
		$out .= '</div>';
		return $out;
...
if ( ! function_exists( 'dailyos_open_loops_feed_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the open-loops-feed projection.
	 ...
	 */
	function dailyos_open_loops_feed_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_open_loops_feed_render_row' ) ) {
	/**
	 * Render a single claim row inside the open-loops-feed projection.
	 ...
	 * @return string
	 */
	function dailyos_open_loops_feed_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<article class="wp-block-dailyos-open-loops-feed__row" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			. '<span class="wp-block-dailyos-open-loops-feed__row-label">' . esc_html( $claim_id ) . '</span>'
			. '</article>';
	}
}
