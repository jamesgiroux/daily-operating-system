...
if ( ! function_exists( 'dailyos_account_detail_open_loops_feed_render' ) ) {
	/**
	 ...
	 * @return string
	 */
	function dailyos_account_detail_open_loops_feed_render( array $attributes, string $content = '', $block = null ): string {
		...
		$projected_claim_refs = dailyos_account_detail_open_loops_feed_select_claim_refs( $envelope );
		$rows = '';
		foreach ( $projected_claim_refs as $claim_ref ) {
			...
			$rows .= dailyos_account_detail_open_loops_feed_render_row( $claim_ref, $receipt );
		}
		$out .= '' !== $rows
			? $rows
			: dailyos_empty_chip( 'no_open_loops', __( 'No open loops yet', 'dailyos' ), 'RecommendedActions_root' );
		$out .= '</div>';
		$out .= '</section>';
		return $out;
	}
...
if ( ! function_exists( 'dailyos_account_detail_open_loops_feed_select_claim_refs' ) ) {
	/**
	 ...
	 */
	function dailyos_account_detail_open_loops_feed_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_account_detail_open_loops_feed_render_row' ) ) {
	/**
	...
	function dailyos_account_detail_open_loops_feed_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<div class="RecommendedActions_action" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			...
			. '<div class="RecommendedActions_title">' . esc_html( $display ) . '</div>'
