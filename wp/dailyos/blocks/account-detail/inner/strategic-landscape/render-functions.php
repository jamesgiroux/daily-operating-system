<?php
/**
 * Strategic Landscape (strategic-landscape) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (landscape narrative).
 *
...
if ( ! function_exists( 'dailyos_strategic_landscape_render' ) ) {
	/**
	 * Render the strategic-landscape inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_strategic_landscape_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_strategic_landscape_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the strategic-landscape projection.
	 ...
	 */
	function dailyos_strategic_landscape_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			$envelope,
			[ 'facts' ],
			[
				'field_paths'         => [ 'companyContext' ],
				'field_path_prefixes' => [ 'strategicPriorities[', 'marketContext[', 'regulatoryContext[' ],
			]
		);
	}
}

if ( ! function_exists( 'dailyos_strategic_landscape_render_row' ) ) {
	/**
	 * Render a single claim row inside the strategic-landscape projection.
	...
	function dailyos_strategic_landscape_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<article class="StrategicLandscape_priorityCard" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			...
			. '<div class="StrategicLandscape_priorityName">' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</div>'
