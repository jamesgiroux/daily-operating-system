<?php
/**
 * Outlook Panel (outlook-panel) — W2 L1 inner block render-functions.
 *
 * Projection rule: Health.outlook factor + renewalCallVerdict projection.
 *
...
if ( ! function_exists( 'dailyos_outlook_panel_render' ) ) {
	/**
	 * Render the outlook-panel inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_outlook_panel_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_outlook_panel_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the outlook-panel projection.
	 ...
	 */
	function dailyos_outlook_panel_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			$envelope,
			[ 'facts' ],
			[
				'field_paths'         => [ 'agreementOutlook', 'contractContext' ],
...
if ( ! function_exists( 'dailyos_outlook_panel_render_row' ) ) {
	/**
	 * Render a single claim row inside the outlook-panel projection.
	...
	function dailyos_outlook_panel_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<div class="AccountOutlook_signalItem" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			...
			. '<p class="AccountOutlook_signalText">' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</p>'
