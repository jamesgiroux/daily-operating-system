<?php
/**
 * Stakeholder Grid (stakeholder-grid) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (people) ∪ Touchpoints (last-touch per person).
 *
...
if ( ! function_exists( 'dailyos_stakeholder_grid_render' ) ) {
	/**
	 * Render the stakeholder-grid inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_stakeholder_grid_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_stakeholder_grid_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the stakeholder-grid projection.
	 ...
	 */
	function dailyos_stakeholder_grid_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_stakeholder_grid_render_row' ) ) {
	/**
	 * Render a single claim row inside the stakeholder-grid projection.
