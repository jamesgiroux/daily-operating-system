<?php
/**
 * Supporting Tension (supporting-tension) — W2 L1 inner block render-functions.
 *
 * Projection rule: Health composed factor delta.
 *
...
if ( ! function_exists( 'dailyos_supporting_tension_render' ) ) {
	/**
	 * Render the supporting-tension inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_supporting_tension_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			$reason = '' !== $first_empty_reason ? $first_empty_reason : 'no_tension_signal';
			return dailyos_empty_chip(
				$reason,
				__( 'No supporting tension on file', 'dailyos' ),
				'wp-block-dailyos-supporting-tension'
			);
		}

		$wrapper_attrs = dailyos_inner_block_wrapper_attrs( 'wp-block-dailyos-supporting-tension' );
		$out  = '<div ' . $wrapper_attrs . ' data-dailyos-projection="supporting-tension">';
		$out .= '<header class="wp-block-dailyos-supporting-tension__header"><span class="wp-block-dailyos-supporting-tension__title">' . esc_html__( 'Supporting Tension', 'dailyos' ) . '</span></header>';
		$out .= '<div class="wp-block-dailyos-supporting-tension__body" data-dailyos-envelope-sections="' . esc_attr( implode( ',', ['health'] ) ) . '">';
		...
		$projected_claim_refs = dailyos_supporting_tension_select_claim_refs( $envelope );
		foreach ( $projected_claim_refs as $claim_ref ) {
			$receipt = dailyos_envelope_consume_claim( $claim_ref, $scope_set );
			if ( null === $receipt ) {
				continue;
			}
			$out .= dailyos_supporting_tension_render_row( $claim_ref, $receipt );
		}
		$out .= '</div>';
		$out .= '</div>';
		return $out;
...
if ( ! function_exists( 'dailyos_supporting_tension_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the supporting-tension projection.
	 ...
	 */
	function dailyos_supporting_tension_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_supporting_tension_render_row' ) ) {
	/**
	 * Render a single claim row inside the supporting-tension projection.
	...
	function dailyos_supporting_tension_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<article class="wp-block-dailyos-supporting-tension__row" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			. '<span class="wp-block-dailyos-supporting-tension__row-label">' . esc_html( $claim_id ) . '</span>'
			. '</article>';
	}
}
