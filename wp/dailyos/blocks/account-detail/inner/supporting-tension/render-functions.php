...
if ( ! function_exists( 'dailyos_supporting_tension_render' ) ) {
	...
	function dailyos_supporting_tension_render( array $attributes, string $content = '', $block = null ): string {
		...
		$out  = '<section id="relationship-health" class="entity-detail_chapterSection" data-ds-tier="pattern" data-ds-name="SupportingTension" data-ds-spec="patterns/SupportingTension.md">';
		...
		$out .= '<div class="health_dimensions">';
		...
		foreach ( $projected_claim_refs as $claim_ref ) {
			...
			$rows .= dailyos_supporting_tension_render_row( $claim_ref, $receipt );
		}
		$out .= '' !== $rows
			? $rows
			: dailyos_empty_chip( 'no_tension_signal', __( 'No supporting tension on file', 'dailyos' ), 'health_dimensions' );
		$out .= '</div>';
		$out .= '</section>';
		return $out;
	}
...
if ( ! function_exists( 'dailyos_supporting_tension_select_claim_refs' ) ) {
	...
	function dailyos_supporting_tension_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			...
			[ 'facts' ],
			[
				'field_paths' => [ 'health', 'coverageAssessment', 'meetingCadence', 'emailResponsiveness' ],
			]
		);
	}
}
...
if ( ! function_exists( 'dailyos_supporting_tension_render_row' ) ) {
	...
	function dailyos_supporting_tension_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<div class="health_dim" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			. '<div class="health_dimName"><span>' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</span><span class="health_dimScore">' . esc_html( $trust_band ) . '</span></div>'
