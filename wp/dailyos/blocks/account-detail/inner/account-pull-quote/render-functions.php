<?php
/**
 * Account Pull Quote (account-pull-quote) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (thesis narrative quote).
 *
...
if ( ! function_exists( 'dailyos_account_pull_quote_render' ) ) {
	/**
	 * Render the account-pull-quote inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_account_pull_quote_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_account_pull_quote_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the account-pull-quote projection.
	 ...
	 */
	function dailyos_account_pull_quote_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			$envelope,
			[ 'facts' ],
			[ 'field_paths' => [ 'pullQuote', 'executiveAssessment' ] ]
		);
	}
}

if ( ! function_exists( 'dailyos_account_pull_quote_render_row' ) ) {
	/**
	 * Render a single claim row inside the account-pull-quote projection.
