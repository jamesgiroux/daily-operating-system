	...
	function dailyos_account_hero_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			return dailyos_empty_chip(
				...
				'wp-block-dailyos-account-hero'
			);
		}

		$wrapper_attrs = dailyos_inner_block_wrapper_attrs( 'wp-block-dailyos-account-hero' );
		$out  = '<div ' . $wrapper_attrs . ' data-dailyos-projection="account-hero">';
		$out .= '<header class="wp-block-dailyos-account-hero__header"><span class="wp-block-dailyos-account-hero__title">' . esc_html__( 'Account Hero', 'dailyos' ) . '</span></header>';
		$out .= '<div class="wp-block-dailyos-account-hero__body" data-dailyos-envelope-sections="' . esc_attr( implode( ',', ['facts'] ) ) . '">';
...
if ( ! function_exists( 'dailyos_account_hero_render_row' ) ) {
	/**
	 * Render a single claim row inside the account-hero projection.
	...
	function dailyos_account_hero_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<article class="wp-block-dailyos-account-hero__row" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			. '<span class="wp-block-dailyos-account-hero__row-label">' . esc_html( $claim_id ) . '</span>'
			. '</article>';
	}
}
