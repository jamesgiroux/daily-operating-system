<?php
/**
 * Linear Issues Chapter (linear-issues-chapter) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (external Linear refs).
 *
...
if ( ! function_exists( 'dailyos_linear_issues_chapter_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the linear-issues-chapter projection.
	 ...
	 */
	function dailyos_linear_issues_chapter_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_linear_issues_chapter_render_row' ) ) {
	/**
	 * Render a single claim row inside the linear-issues-chapter projection.
	...
	function dailyos_linear_issues_chapter_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<div class="EntityRow_row EntityRow_rowBorder" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			...
			. '<div class="EntityRow_nameRow"><span class="EntityRow_name">' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</span></div>'
