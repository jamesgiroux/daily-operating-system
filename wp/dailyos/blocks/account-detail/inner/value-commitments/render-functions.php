<?php
/**
 * Value Commitments (value-commitments) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (commitments narrative).
 *
...
if ( ! function_exists( 'dailyos_value_commitments_render' ) ) {
	/**
	 * Render the value-commitments inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_value_commitments_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_value_commitments_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the value-commitments projection.
	 ...
	 */
	function dailyos_value_commitments_select_claim_refs( ?array $envelope ): array {
		return dailyos_envelope_collect_claim_refs(
			$envelope,
			[ 'facts' ],
			[
				'field_path_prefixes' => [ 'valueDelivered[', 'successMetrics[', 'openCommitments[' ],
			]
		);
	}
}

if ( ! function_exists( 'dailyos_value_commitments_render_row' ) ) {
	/**
	 * Render a single claim row inside the value-commitments projection.
	...
	function dailyos_value_commitments_render_row( array $claim_ref, array $receipt ): string {
		...
		return '<article class="ValueCommitments_valueCard" data-claim-id="' . esc_attr( $claim_id ) . '" data-trust-band="' . esc_attr( $trust_band ) . '">'
			...
			. '<div class="ValueCommitments_valueHeadline">' . esc_html( dailyos_receipt_rendered_text( $receipt, $claim_id ) ) . '</div>'
			. '</article>';
	}
}
