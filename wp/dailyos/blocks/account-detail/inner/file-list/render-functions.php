<?php
/**
 * File List (file-list) — W2 L1 inner block render-functions.
 *
 * Projection rule: Facts (attached file refs).
 *
...
if ( ! function_exists( 'dailyos_file_list_render' ) ) {
	/**
	 * Render the file-list inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_file_list_render( array $attributes, string $content = '', $block = null ): string {
...
if ( ! function_exists( 'dailyos_file_list_select_claim_refs' ) ) {
	/**
	 * Select claim references from the envelope for the file-list projection.
	 ...
	 */
	function dailyos_file_list_select_claim_refs( ?array $envelope ): array {
		if ( null === $envelope ) {
			return [];
		}
		$refs = [];
...
if ( ! function_exists( 'dailyos_file_list_render_row' ) ) {
	/**
	 * Render a single claim row inside the file-list projection.
