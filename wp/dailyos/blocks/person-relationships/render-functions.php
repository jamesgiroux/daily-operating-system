<?php
/**
 * Person Relationships inner-block server-side render (W2 L1 — DOS-484).
 *
 * Projects from envelope section(s): Facts (relationship narrative) + Touchpoints (per-relationship cadence).
 * Trust band source: per-relationship.
 *
...
if ( ! function_exists( 'dailyos_person_relationships_render' ) ) {
	/**
	 * Render the Person Relationships inner block.
	 *
	 ...
	 */
	function dailyos_person_relationships_render( array $attributes, array $ctx = [] ): string {
		...
		$wrapper_attrs = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'        => 'wp-block-dailyos-person-relationships',
		...
		$out .= '<div class="dailyos-person-relationships__body" data-dailyos-projection="person-relationships" data-trust-band-source="per-relationship"></div>';
		$out .= '</section>';
