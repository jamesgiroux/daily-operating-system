<?php
/**
 * Person Network inner-block server-side render (W2 L1 — DOS-484).
 *
 * Projects from envelope section(s): Facts (network refs to accounts/projects/people).
 * Trust band source: per-edge.
 *
...
if ( ! function_exists( 'dailyos_person_network_render' ) ) {
	/**
	 * Render the Person Network inner block.
	 *
	 ...
	 */
	function dailyos_person_network_render( array $attributes, array $ctx = [] ): string {
		...
		$wrapper_attrs = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'        => 'wp-block-dailyos-person-network',
		...
		$out .= '<div class="dailyos-person-network__body" data-dailyos-projection="person-network" data-trust-band-source="per-edge"></div>';
		$out .= '</section>';
