<?php
/**
 * MeetingContextBundle inner-block server-side render (W2 §5.4 / DOS-752).
 *
 * Projection: Facts (referenced entity context) + envelope-level\n * `EnvelopeProvenance`. Aggregate trust band. Links into Account/\n * Project/Person detail composites.
 *
 * @package DailyOS
 */
	...
	function dailyos_meeting_context_bundle_render( array $attributes, $block = null ): string {
		...
		if ( ! is_object( $runtime_client ) || ! method_exists( $runtime_client, 'invoke_ability' ) ) {
			return '<span class="dailyos-empty-chip" data-empty-reason="runtime_unavailable">'
				. esc_html__( 'Context bundle unavailable.', 'dailyos' )
				. '</span>';
		}
		...
		$wrapper_attrs = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'        => 'wp-block-dailyos-meeting-context-bundle',
					'data-ds-tier' => 'primitive',
					'data-ds-name' => 'MeetingContextBundle',
				]
			)
			: 'class="wp-block-dailyos-meeting-context-bundle"';

		return '<section ' . $wrapper_attrs . ' data-empty-reason="">'
			. '<h2 class="wp-block-dailyos-meeting-context-bundle__title">'
			. esc_html__( 'Context bundle', 'dailyos' )
			. '</h2>'
			. '</section>';
	}
}
