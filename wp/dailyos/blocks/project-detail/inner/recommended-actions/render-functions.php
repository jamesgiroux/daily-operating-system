<?php
/**
 * Recommended Actions (recommended-actions) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: open_loops (envelope sections projected by this chapter).
 * Trust band source: per_action.
 *
...
if ( ! function_exists( 'dailyos_recommended_actions_render' ) ) {
	...
	function dailyos_recommended_actions_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No recommended actions', 'dailyos' ),
					'wp-block-dailyos-recommended-actions'
				);
			}
			return '<section class="wp-block-dailyos-recommended-actions"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No recommended actions', 'dailyos' ) . '</span></section>';
		}
		...
		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					...
					'data-ds-name'                => 'recommended-actions',
					'data-dailyos-trust-band-src' => 'per_action',
				]
			)
			: 'class="wp-block-dailyos-recommended-actions" data-dailyos-trust-band-src="per_action"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-recommended-actions-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
