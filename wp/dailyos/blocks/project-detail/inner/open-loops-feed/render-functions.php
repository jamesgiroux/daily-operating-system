<?php
/**
 * Open Loops Feed (open-loops-feed) — W2 L1 inner block render-functions (DOS-483).
 *
...
if ( ! function_exists( 'dailyos_open_loops_feed_render' ) ) {
	...
	function dailyos_open_loops_feed_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No open loops', 'dailyos' ),
					'wp-block-dailyos-open-loops-feed'
				);
			}
			return '<section class="wp-block-dailyos-open-loops-feed"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No open loops', 'dailyos' ) . '</span></section>';
		}
		...
		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					...
					'data-ds-name'                => 'open-loops-feed',
					'data-dailyos-trust-band-src' => 'per_loop',
				]
			)
			: 'class="wp-block-dailyos-open-loops-feed" data-dailyos-trust-band-src="per_loop"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-open-loops-feed-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
