<?php
/**
 * Portfolio Chapter (portfolio-chapter) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts (envelope sections projected by this chapter).
 * Trust band source: per_child_aggregate.
 *
...
if ( ! function_exists( 'dailyos_portfolio_chapter_render' ) ) {
	/**
	 * Render the portfolio-chapter inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_portfolio_chapter_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'Leaf project — no portfolio', 'dailyos' ),
					'wp-block-dailyos-portfolio-chapter'
				);
			}
			return '<section class="wp-block-dailyos-portfolio-chapter"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'Leaf project — no portfolio', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-portfolio-chapter',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'portfolio-chapter',
					'data-dailyos-trust-band-src' => 'per_child_aggregate',
				]
			)
			: 'class="wp-block-dailyos-portfolio-chapter" data-dailyos-trust-band-src="per_child_aggregate"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-portfolio-chapter-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
