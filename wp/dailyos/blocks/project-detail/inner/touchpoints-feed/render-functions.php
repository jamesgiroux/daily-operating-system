<?php
/**
 * Touchpoints Feed (touchpoints-feed) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: touchpoints (envelope sections projected by this chapter).
 * Trust band source: per_touchpoint.
 *
...
if ( ! function_exists( 'dailyos_project_detail_touchpoints_feed_render' ) ) {
	/**
	 ...
	 * @return string
	 */
	function dailyos_project_detail_touchpoints_feed_render( array $attributes, string $content = '', $block = null ): string {
		...
		// AgentMcp audience filter (W2 V1.2.1 §5.2 projection-table note):
		// when the resolved audience is agent_mcp, this chapter renders
		// aggregate-only (no per-touchpoint rows). The audience comes from
		// the actor / surface-client resolution; default = user.
		$audience = '';
		if ( null !== $block && isset( $block->context['dailyos/audience'] ) ) {
			$audience = (string) $block->context['dailyos/audience'];
		}
		$aggregate_only = ( 'agent_mcp' === $audience );
		...
		if ( ! $any_present ) {
			...
			return '<section class="wp-block-dailyos-touchpoints-feed"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No touchpoints yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                          => 'wp-block-dailyos-touchpoints-feed' . ( $aggregate_only ? ' is-aggregate-only' : '' ),
					'data-ds-tier'                   => 'pattern',
					'data-ds-name'                   => 'touchpoints-feed',
					'data-dailyos-trust-band-src'    => 'per_touchpoint',
					'data-dailyos-audience-shape'    => $aggregate_only ? 'aggregate' : 'rows',
				]
			)
			: 'class="wp-block-dailyos-touchpoints-feed' . ( $aggregate_only ? ' is-aggregate-only' : '' ) . '" data-dailyos-trust-band-src="per_touchpoint" data-dailyos-audience-shape="' . ( $aggregate_only ? 'aggregate' : 'rows' ) . '"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-touchpoints-feed-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
