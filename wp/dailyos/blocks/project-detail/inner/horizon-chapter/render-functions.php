<?php
/**
 * Horizon Chapter (horizon-chapter) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts,health (envelope sections projected by this chapter).
...
if ( ! function_exists( 'dailyos_horizon_chapter_render' ) ) {
	/**
	 * Render the horizon-chapter inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_horizon_chapter_render( array $attributes, string $content = '', $block = null ): string {
		...
		$projected_sections = [ 'facts', 'health' ];
		$any_present        = false;
		$first_empty_reason = '';
		...
		if ( ! $any_present ) {
			$reason = '' !== $first_empty_reason ? $first_empty_reason : 'no_horizon_read';
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No horizon read yet', 'dailyos' ),
					'wp-block-dailyos-horizon-chapter'
				);
			}
			return '<section class="wp-block-dailyos-horizon-chapter"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No horizon read yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-horizon-chapter',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'horizon-chapter',
					'data-dailyos-trust-band-src' => 'aggregate',
				]
			)
			: 'class="wp-block-dailyos-horizon-chapter" data-dailyos-trust-band-src="aggregate"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-horizon-chapter-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
