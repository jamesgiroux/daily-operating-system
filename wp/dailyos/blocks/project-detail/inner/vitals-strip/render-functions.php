<?php
/**
 * Vitals Strip (vitals-strip) — W2 L1 inner block render-functions (DOS-483).
 *
...
if ( ! function_exists( 'dailyos_vitals_strip_render' ) ) {
	 ...
	 * @return string
	 */
	function dailyos_vitals_strip_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No vitals signal yet', 'dailyos' ),
					'wp-block-dailyos-vitals-strip'
				);
			}
			return '<section class="wp-block-dailyos-vitals-strip"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No vitals signal yet', 'dailyos' ) . '</span></section>';
		}
		...
		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					...
					'data-ds-name'                => 'vitals-strip',
					'data-dailyos-trust-band-src' => 'aggregate',
				]
			)
			: 'class="wp-block-dailyos-vitals-strip" data-dailyos-trust-band-src="aggregate"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-vitals-strip-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
