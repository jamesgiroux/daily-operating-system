<?php
/**
 * Linear Issues Chapter (linear-issues-chapter) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts (envelope sections projected by this chapter).
 * Trust band source: per_issue.
 *
...
if ( ! function_exists( 'dailyos_linear_issues_chapter_render' ) ) {
	/**
	 * Render the linear-issues-chapter inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_linear_issues_chapter_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			return '<section class="wp-block-dailyos-linear-issues-chapter"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No Linear issues linked', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-linear-issues-chapter',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'linear-issues-chapter',
					'data-dailyos-trust-band-src' => 'per_issue',
				]
			)
			: 'class="wp-block-dailyos-linear-issues-chapter" data-dailyos-trust-band-src="per_issue"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-linear-issues-chapter-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
