<?php
/**
 * Project Hero (project-hero) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts (envelope sections projected by this chapter).
 * Trust band source: none.
 *
...
if ( ! function_exists( 'dailyos_project_hero_render' ) ) {
	/**
	 * Render the project-hero inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_project_hero_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			$reason = '' !== $first_empty_reason ? $first_empty_reason : 'no_project_facts';
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No project facts yet', 'dailyos' ),
					'wp-block-dailyos-project-hero'
				);
			}
			return '<section class="wp-block-dailyos-project-hero"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No project facts yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-project-hero',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'project-hero',
					'data-dailyos-trust-band-src' => 'none',
				]
			)
			: 'class="wp-block-dailyos-project-hero" data-dailyos-trust-band-src="none"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-project-hero-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
