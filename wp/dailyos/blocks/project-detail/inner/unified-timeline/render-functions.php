<?php
/**
 * Unified Timeline (unified-timeline) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: record,metadata_proposals (envelope sections projected by this chapter).
 * Trust band source: per_entry.
 *
...
if ( ! function_exists( 'dailyos_project_detail_unified_timeline_render' ) ) {
	 ...
	 * @return string
	 */
	function dailyos_project_detail_unified_timeline_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No timeline entries yet', 'dailyos' ),
					'wp-block-dailyos-unified-timeline'
				);
			}
			return '<section class="wp-block-dailyos-unified-timeline"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No timeline entries yet', 'dailyos' ) . '</span></section>';
		}
		...
		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					...
					'data-ds-name'                => 'unified-timeline',
					'data-dailyos-trust-band-src' => 'per_entry',
				]
			)
			: 'class="wp-block-dailyos-unified-timeline" data-dailyos-trust-band-src="per_entry"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-unified-timeline-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
