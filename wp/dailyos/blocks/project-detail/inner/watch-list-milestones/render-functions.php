<?php
/**
 * Watch List Milestones (watch-list-milestones) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts,open_loops (envelope sections projected by this chapter).
 * Trust band source: per_milestone.
 *
...
if ( ! function_exists( 'dailyos_watch_list_milestones_render' ) ) {
	/**
	 * Render the watch-list-milestones inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_watch_list_milestones_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No milestones yet', 'dailyos' ),
					'wp-block-dailyos-watch-list-milestones'
				);
			}
			return '<section class="wp-block-dailyos-watch-list-milestones"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No milestones yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-watch-list-milestones',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'watch-list-milestones',
					'data-dailyos-trust-band-src' => 'per_milestone',
				]
			)
			: 'class="wp-block-dailyos-watch-list-milestones" data-dailyos-trust-band-src="per_milestone"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-watch-list-milestones-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
