<?php
/**
 * The Work (the-work) — W2 L1 inner block render-functions (DOS-483).
 *
 * Projection rule: facts (envelope sections projected by this chapter).
 * Trust band source: per_work_item.
 *
...
if ( ! function_exists( 'dailyos_the_work_render' ) ) {
	/**
	 * Render the the-work inner block.
	 *
	 ...
	 * @return string
	 */
	function dailyos_the_work_render( array $attributes, string $content = '', $block = null ): string {
		...
		if ( ! $any_present ) {
			...
			if ( function_exists( 'dailyos_empty_chip' ) ) {
				return dailyos_empty_chip(
					$reason,
					__( 'No attached work yet', 'dailyos' ),
					'wp-block-dailyos-the-work'
				);
			}
			return '<section class="wp-block-dailyos-the-work"><span class="dailyos-empty-chip" data-empty-reason="' . esc_attr( $reason ) . '">' . esc_html__( 'No attached work yet', 'dailyos' ) . '</span></section>';
		}

		$wrapper = function_exists( 'get_block_wrapper_attributes' )
			? get_block_wrapper_attributes(
				[
					'class'                       => 'wp-block-dailyos-the-work',
					'data-ds-tier'                => 'pattern',
					'data-ds-name'                => 'the-work',
					'data-dailyos-trust-band-src' => 'per_work_item',
				]
			)
			: 'class="wp-block-dailyos-the-work" data-dailyos-trust-band-src="per_work_item"';
		...
		$out  = '<section ' . $wrapper . '>';
		$out .= '<div class="dailyos-the-work-body"></div>';
		$out .= '</section>';
		return $out;
	}
}
